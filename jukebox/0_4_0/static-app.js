...
function peerIdForRoom(code) {
  return `${config.peerPrefix || 'gnj-v040-'}${String(code || '').toLowerCase()}`;
}
...
function renderChat(messages = []) {
  refs.chatCount.textContent = String(messages.length);
  refs.chatList.replaceChildren();
  if (!messages.length) {
    refs.chatList.append(el('div', 'empty-state', 'No messages yet.'));
    return;
  }
  for (const message of messages) {
    ...
    refs.chatList.append(row);
  }
  refs.chatList.scrollTop = refs.chatList.scrollHeight || 0;
}
...
refs.chatForm.addEventListener('submit', async (event) => {
  ...
  const submit = refs.chatForm.querySelector?.('button[type="submit"]');
