(() => {
  ...
  const state = {
    ...
    mediaCarrier: null, mediaStreamActive: false, mediaLastKeepAliveAt: 0, mediaMetadataFingerprint: '',
  ...
  async function initBackendBridge() {
    ...
    try {
      ...
      const backendCompatible = !info.version || ['0.2.0','0.2.1','0.2.2','0.2.3'].includes(info.version);
  ...
  function updateMediaSession() {
    ...
    const metadataFingerprint = JSON.stringify([officeId, office, status, myStatus, peopleCount]);
    if ('MediaMetadata' in window && metadataFingerprint !== state.mediaMetadataFingerprint) {
      try {
        ...
        navigator.mediaSession.metadata = new MediaMetadata({
          ...
          album: `Play keeps live refresh active • Previous lowers • Next raises • ${peopleCount} ${peopleCount === 1 ? 'person' : 'people'} checked in`,
          artwork: [
