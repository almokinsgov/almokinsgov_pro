...
async function init() {
  ...
  bindEvents();
  if (els.submissionStatusFilter) els.submissionStatusFilter.value = '';
  loadSettingsForm();
  applyLookupSettingState();
  setEditorMode('guided');
  setStep('what');
  setBusy(true, 'Loading rāhui and submissions', 'Syncing records and checking for public submissions from the configured storage.');
  try {
    await Promise.all([
      reloadRecords(),
