...
const SUBMISSION_STATUS_VALUES = new Set(['pending', 'reviewing', 'converted', 'declined']);
...
export function createSubmissionId() {
  return createRahuiId();
}

export function createBlankSubmission(overrides = {}) {
  ...
  const base = {
    ...
    rahui: createBlankRahui({ id: '', published: false, archived: false }),
    ...overrides
  };
...
export function normalizeSubmission(input = {}) {
  ...
  item.status = SUBMISSION_STATUS_VALUES.has(item.status) ? item.status : 'pending';
  item.submittedAt = String(item.submittedAt || '').trim();
  item.updatedAt = String(item.updatedAt || item.submittedAt || new Date().toISOString());
  item.reviewedAt = String(item.reviewedAt || '').trim();
  item.reviewedBy = String(item.reviewedBy || '').trim();
  item.reviewNotes = String(item.reviewNotes || '').trim();
  item.rahuiId = String(item.rahuiId || '').trim();
  item.submitterName = String(item.submitterName || '').trim();
  item.submitterEmail = String(item.submitterEmail || '').trim();
  item.submitterPhone = String(item.submitterPhone || '').trim();
  item.reviewAcknowledged = toBoolean(item.reviewAcknowledged);
  item.rahui = normalizeRahui({ ...(item.rahui || {}), id: '' });
  item.rahui.published = false;
  item.rahui.archived = false;
  return item;
}
...
export function validateSubmission(input) {
  ...
  const errors = [];
  if (!item.id) errors.push('A submission ID is required.');
  if (!item.submitterName) errors.push('Your name is required.');
  if (!item.submitterEmail && !item.submitterPhone) errors.push('Provide an email address or phone number so the review team can contact you.');
  if (item.submitterEmail && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(item.submitterEmail)) errors.push('Enter a valid email address or leave the email field blank.');
  if (item.submitterPhone && item.submitterPhone.replace(/\D/g, '').length < 6) errors.push('Enter a valid phone number or leave the phone field blank.');
  if (!item.reviewAcknowledged) errors.push('Please confirm that you understand the submission will be reviewed before publication.');
  if (!item.rahui.title) errors.push('A rāhui title is required.');
  if (!item.rahui.geometry) errors.push('Draw the affected rāhui area on the map.');
  if (item.rahui.startDate && item.rahui.endDate && item.rahui.endDate < item.rahui.startDate) errors.push('End date cannot be before start date.');
  if (item.rahui.liftedDate && item.rahui.startDate && item.rahui.liftedDate < item.rahui.startDate) errors.push('Lifted date cannot be before start date.');
  return errors;
}

export function submissionToDraftRahui(input) {
  const submission = normalizeSubmission(input);
  const id = submission.rahuiId || submission.id || createRahuiId();
  const note = `Created from rāhui submission ${submission.id}. Submission contact details remain in the submissions register and are not public contact details.`;
  return normalizeRahui({
    ...submission.rahui,
    id,
    published: false,
    archived: false,
    internalNotes: [submission.rahui.internalNotes, note].filter(Boolean).join('\n\n'),
    version: 1
  });
}
