...
const SCREEN_EVENT_TYPES = new Set(['discoBall', 'confetti', 'laserShow']);
...
const mockCollections = {
  playlists: [
    { id: 'mock-playlist-party', type: 'playlist', name: 'Game Night Warm-up', description: 'Host playlist', owner: 'Host', image: '', uri: 'spotify:playlist:mock-playlist-party', tracks: mockTracks.slice(0, 6) },
    { id: 'mock-playlist-late', type: 'playlist', name: 'Late Lobby', description: 'After-hours favourites', owner: 'Host', image: '', uri: 'spotify:playlist:mock-playlist-late', tracks: mockTracks.slice(6, 12) }
  ],
  albums: [
    { id: 'mock-album-checkpoint', type: 'album', name: 'Checkpoint Radio', description: 'Pixel Raiders', owner: 'Pixel Raiders', image: '', uri: 'spotify:album:mock-album-checkpoint', tracks: mockTracks.slice(0, 4) },
    { id: 'mock-album-shared-xp', type: 'album', name: 'Shared XP', description: 'Co-op Club', owner: 'Co-op Club', image: '', uri: 'spotify:album:mock-album-shared-xp', tracks: mockTracks.slice(5, 9) }
  ]
};
...
const state = {
  ...
  lastSpotifyAutoSyncAt: 0,
  spotifyAutoSyncBusy: false,
  silentDiscoCapture: null,
  silentDiscoStream: null,
  silentDiscoCalls: new Map(),
  silentDiscoIncomingCall: null,
  silentDiscoRemoteStream: null,
  silentDiscoListening: false
};
...
const refs = {
  ...
  spotifySessionActions: $('#spotifySessionActions'),
  refreshSpotifySigninButton: $('#refreshSpotifySigninButton'),
  signOutSpotifyButton: $('#signOutSpotifyButton'),
  ...
  showHostListeningInput: $('#showHostListeningInput'),
  allowLibraryViewingInput: $('#allowLibraryViewingInput'),
  hostRequestModeInput: $('#hostRequestModeInput'),
  hostQueueModeInput: $('#hostQueueModeInput'),
  hostCooldownInput: $('#hostCooldownInput'),
  hostMessage: $('#hostMessage'),
  spotifyLibraryPanel: $('#spotifyLibraryPanel'),
  spotifyLibraryTitle: $('#spotifyLibraryTitle'),
  spotifyLibraryBadge: $('#spotifyLibraryBadge'),
  spotifyLibraryMessage: $('#spotifyLibraryMessage'),
  spotifyLibraryTabs: $('#spotifyLibraryTabs'),
  loadPlaylistsButton: $('#loadPlaylistsButton'),
  loadAlbumsButton: $('#loadAlbumsButton'),
  ...
  openPipButton: $('#openPipButton'),
  reactionButtons: [...(document.querySelectorAll?.('.reaction-button') || [])],
  screenEventButtons: [...(document.querySelectorAll?.('.screen-event-button') || [])],
  silentDiscoHostBlock: $('#silentDiscoHostBlock'),
  silentDiscoHostStatus: $('#silentDiscoHostStatus'),
  startSilentDiscoButton: $('#startSilentDiscoButton'),
  stopSilentDiscoButton: $('#stopSilentDiscoButton'),
  silentDiscoGuestPanel: $('#silentDiscoGuestPanel'),
  silentDiscoBadge: $('#silentDiscoBadge'),
  silentDiscoGuestStatus: $('#silentDiscoGuestStatus'),
  silentDiscoAudio: $('#silentDiscoAudio'),
  joinSilentDiscoButton: $('#joinSilentDiscoButton'),
  leaveSilentDiscoButton: $('#leaveSilentDiscoButton'),
  reactionLayer: $('#reactionLayer'),
  screenEventLayer: $('#screenEventLayer'),
  toast: $('#toast')
};
...
function peerIdForRoom(code) {
  return `${config.peerPrefix || 'gnj-v051-'}${String(code || '').toLowerCase()}`;
}
...
function spotifyPlaylistAccessNeedsRefresh() {
  const scope = String(spotifyTokenRecord()?.scope || '');
  return !MOCK_MODE && spotifyConnected() && scope && !scope.split(/\s+/).includes('playlist-read-private');
}

function publicRoom(room, viewer = { isHost: true }) {
  ...
  const externalPlaying = Boolean(room.nowPlaying && ['spotify', 'host-listening', 'host-library'].includes(room.nowPlaying.source));
  const canSeeHostListening = isHost || room.showHostListening === true || !externalPlaying;
  const history = isHost || room.showHostListening === true ? room.history : room.history.filter((item) => !['spotify', 'host-listening', 'host-library'].includes(item.source));
  return {
    code: room.code,
    ...
    showHostListening: room.showHostListening === true,
    allowLibraryViewing: room.allowLibraryViewing === true,
    silentDiscoActive: room.silentDiscoActive === true,
    hostListeningHidden: !canSeeHostListening,
    featuredCollection: isHost || room.allowLibraryViewing === true ? room.featuredCollection : null,
    selectedDeviceId: room.selectedDeviceId,
    selectedDeviceName: room.selectedDeviceName,
    nowPlaying: canSeeHostListening ? room.nowPlaying : null,
    history,
    chat: room.chat || [],
    queue,
    suggestions,
...
function loadPersistedHostRoom(code = '') {
  try {
    ...
    return {
      ...
      showHostListening: raw.showHostListening === true,
      allowLibraryViewing: raw.allowLibraryViewing === true,
      silentDiscoActive: false,
      trackCache: new Map()
    };
  } catch {
    return null;
...
function createRoomModel({ code, hostName, pin, requestMode, queueMode, requestCooldownSeconds }) {
  return {
    ...
    showHostListening: false,
    allowLibraryViewing: false,
    silentDiscoActive: false,
...
function publicCollection(collection, { includeTracks = false } = {}) {
  return {
    id: collection.id, type: collection.type, name: collection.name, description: collection.description || '',
    owner: collection.owner || '', image: collection.image || '', uri: collection.uri || '', externalUrl: collection.externalUrl || '',
    trackCount: collection.tracks?.length || collection.trackCount || 0,
    ...(includeTracks ? { tracks: collection.tracks || [] } : {})
  };
}

async function listSpotifyCollections(type) {
  if (MOCK_MODE) return mockCollections[type].map((item) => publicCollection(item));
  if (type === 'playlists') {
    const data = await spotifyFetch('/me/playlists?limit=20');
    return (data.items || []).filter(Boolean).map((item) => ({ id: item.id, type: 'playlist', name: item.name, description: item.description || '', owner: item.owner?.display_name || '', image: item.images?.[0]?.url || '', uri: item.uri || '', externalUrl: item.external_urls?.spotify || '', trackCount: item.items?.total || item.tracks?.total || 0 }));
  }
  const data = await spotifyFetch('/me/albums?limit=20');
  return (data.items || []).map((entry) => entry.album).filter(Boolean).map((item) => ({ id: item.id, type: 'album', name: item.name, description: (item.artists || []).map((artist) => artist.name).join(', '), owner: (item.artists || []).map((artist) => artist.name).join(', '), image: item.images?.[0]?.url || '', uri: item.uri || '', externalUrl: item.external_urls?.spotify || '', trackCount: item.total_tracks || 0 }));
}

async function getSpotifyCollection(type, id) {
  const key = type === 'playlist' ? 'playlists' : 'albums';
  if (MOCK_MODE) {
    const collection = mockCollections[key].find((item) => item.id === id);
    if (!collection) throw new Error('Spotify collection not found.');
    collection.tracks.forEach(cacheTrack);
    return publicCollection(collection, { includeTracks: true });
  }
  if (type === 'playlist') {
    const data = await spotifyFetch(`/playlists/${encodeURIComponent(id)}?fields=id,name,description,images,uri,external_urls,owner(display_name),items(total,items(track)),tracks(total,items(track))`);
    const playlistItems = data.items?.items || data.tracks?.items || [];
    const tracks = playlistItems.map((entry) => entry.track || entry.item).filter(Boolean).map(normaliseSpotifyTrack);
    tracks.forEach(cacheTrack);
    return { id: data.id, type, name: data.name, description: data.description || '', owner: data.owner?.display_name || '', image: data.images?.[0]?.url || '', uri: data.uri || '', externalUrl: data.external_urls?.spotify || '', trackCount: data.items?.total || data.tracks?.total || tracks.length, tracks };
  }
  const data = await spotifyFetch(`/albums/${encodeURIComponent(id)}`);
  const owner = (data.artists || []).map((artist) => artist.name).join(', ');
  const tracks = (data.tracks?.items || []).filter(Boolean).map((track) => normaliseSpotifyTrack({ ...track, album: { name: data.name, images: data.images } }));
  tracks.forEach(cacheTrack);
  return { id: data.id, type, name: data.name, description: owner, owner, image: data.images?.[0]?.url || '', uri: data.uri || '', externalUrl: data.external_urls?.spotify || '', trackCount: data.total_tracks || tracks.length, tracks };
}

async function playSpotifyCollection(collection) {
  const room = state.roomInternal;
  room.featuredCollection = collection;
  if (!collection.tracks?.length) throw new Error('This collection has no playable tracks.');
  if (!MOCK_MODE) {
    const query = room.selectedDeviceId ? `?device_id=${encodeURIComponent(room.selectedDeviceId)}` : '';
    await spotifyFetch(`/me/player/play${query}`, { method: 'PUT', body: JSON.stringify({ context_uri: collection.uri }) });
    await syncPlayback();
    return;
  }
  if (room.nowPlaying) archiveNowPlaying('changed-in-spotify');
  const track = collection.tracks[0];
  room.nowPlaying = { track, requestedBy: 'Spotify', requesterClientId: '', requestItemId: '', startedAt: Date.now(), progressMs: 0, isPlaying: true, source: 'host-library', listeningContext: { type: collection.type, uri: collection.uri, name: collection.name } };
  scheduleAutoNext(track.durationMs);
  roomChanged();
}
...
async function spotifyFetch(endpoint, options = {}, retry = true) {
  ...
  if (!response.ok) {
    ...
    if (response.status === 403) throw new Error(`Spotify denied this request. Refresh the Spotify sign-in; if this app is in Development Mode, add this Spotify account to the app allowlist. ${spotifyMessage}`.trim());
    throw new Error(spotifyMessage || `Spotify request failed (${response.status}).`);
  }
...
function scheduleAutoNext(remainingMs) {
  ...
  state.playbackTimer = setTimeout(() => {
    const external = ['spotify', 'host-listening', 'host-library'].includes(room.nowPlaying?.source);
    const operation = external && room.queue.length === 0 && !MOCK_MODE ? syncPlayback() : playNext('', true);
...
function showScreenEvent(screenEvent) {
  const kind = screenEvent?.kind;
  if (!SCREEN_EVENT_TYPES.has(kind) || !refs.screenEventLayer) return;
  clearTimeout(showScreenEvent.timer);
  state.activeScreenEvent = kind;
  refs.screenEventLayer.className = `screen-event-layer is-active event-${kind}`;
  showScreenEvent.timer = setTimeout(() => {
    state.activeScreenEvent = null;
    refs.screenEventLayer.className = 'screen-event-layer hidden';
  }, Math.max(1200, Math.min(8000, Number(screenEvent.durationMs || 5000))));
}

function publishScreenEvent(kind) {
  if (!state.isHost || !SCREEN_EVENT_TYPES.has(kind)) throw new Error('That big-screen event is not available.');
  const screenEvent = { id: randomToken(12), kind, sentAt: Date.now(), durationMs: kind === 'confetti' ? 3800 : 5200 };
  showScreenEvent(screenEvent);
  for (const conn of state.guestConnections.values()) {
    if (conn?.open) { try { conn.send({ type: 'screenEvent', screenEvent }); } catch {} }
  }
  return screenEvent;
}

function renderSilentDiscoControls() {
  const active = Boolean(state.room?.silentDiscoActive);
  refs.silentDiscoHostBlock.classList.toggle('hidden', !state.isHost);
  refs.silentDiscoGuestPanel.classList.toggle('hidden', state.isHost);
  refs.silentDiscoGuestPanel.classList.toggle('silent-disco-live', active);
  refs.startSilentDiscoButton.disabled = !state.isHost || Boolean(state.silentDiscoStream);
  refs.stopSilentDiscoButton.disabled = !state.isHost || !state.silentDiscoStream;
  if (state.isHost) {
    refs.silentDiscoHostStatus.textContent = state.silentDiscoStream
      ? `Broadcasting live host audio to ${state.silentDiscoCalls.size} connected listener${state.silentDiscoCalls.size === 1 ? '' : 's'}.`
      : 'Choose a browser tab or screen and enable its audio when prompted.';
    return;
  }
  const streamReady = Boolean(state.silentDiscoRemoteStream);
  refs.silentDiscoBadge.textContent = state.silentDiscoListening ? 'Listening' : active ? 'Live' : 'Waiting';
  refs.silentDiscoBadge.classList.toggle('active', active);
  refs.joinSilentDiscoButton.disabled = !streamReady || state.silentDiscoListening;
  refs.leaveSilentDiscoButton.disabled = !state.silentDiscoListening;
  refs.silentDiscoGuestStatus.textContent = state.silentDiscoListening
    ? 'Host audio is playing on this device.'
    : streamReady
      ? 'The host audio stream is ready. Tap Join to enable sound.'
      : active
        ? 'Connecting to the host audio stream…'
        : 'The host has not started an audio broadcast.';
}

function clearSilentDiscoGuestAudio(call = null) {
  if (call && state.silentDiscoIncomingCall !== call) return;
  state.silentDiscoListening = false;
  state.silentDiscoRemoteStream = null;
  try { refs.silentDiscoAudio.pause(); } catch {}
  refs.silentDiscoAudio.srcObject = null;
  state.silentDiscoIncomingCall = null;
  renderSilentDiscoControls();
}

function leaveSilentDiscoAudio() {
  state.silentDiscoListening = false;
  try { refs.silentDiscoAudio.pause(); } catch {}
  renderSilentDiscoControls();
}

function handleIncomingSilentDiscoCall(call) {
  if (state.isHost || call?.metadata?.roomCode !== state.roomCode || call?.metadata?.kind !== 'silent-disco') {
    try { call.close(); } catch {}
    return;
  }
  const previous = state.silentDiscoIncomingCall;
  state.silentDiscoIncomingCall = null;
  try { previous?.close(); } catch {}
  state.silentDiscoIncomingCall = call;
  call.on('stream', (stream) => {
    state.silentDiscoRemoteStream = stream;
    refs.silentDiscoAudio.srcObject = stream;
    renderSilentDiscoControls();
  });
  call.on('close', () => clearSilentDiscoGuestAudio(call));
  call.on('error', () => clearSilentDiscoGuestAudio(call));
  call.answer();
}

function callSilentDiscoGuest(conn) {
  if (!state.isHost || !state.silentDiscoStream || !conn?.peer || typeof state.peer?.call !== 'function') return;
  const previous = state.silentDiscoCalls.get(conn.peer);
  try { previous?.close(); } catch {}
  const call = state.peer.call(conn.peer, state.silentDiscoStream, { metadata: { kind: 'silent-disco', roomCode: state.roomCode } });
  if (!call) return;
  state.silentDiscoCalls.set(conn.peer, call);
  const cleanup = () => {
    if (state.silentDiscoCalls.get(conn.peer) === call) state.silentDiscoCalls.delete(conn.peer);
    renderSilentDiscoControls();
  };
...
function stopSilentDiscoBroadcast() {
  for (const call of state.silentDiscoCalls.values()) { try { call.close(); } catch {} }
  state.silentDiscoCalls.clear();
  const capture = state.silentDiscoCapture;
  state.silentDiscoCapture = null;
  state.silentDiscoStream = null;
  for (const track of capture?.getTracks?.() || []) { try { track.stop(); } catch {} }
  if (state.roomInternal) {
    state.roomInternal.silentDiscoActive = false;
    roomChanged();
  } else renderSilentDiscoControls();
}

async function startSilentDiscoBroadcast() {
  if (!state.isHost || !state.roomInternal) return;
  if (!navigator.mediaDevices?.getDisplayMedia) throw new Error('Silent Disco needs Chrome screen or tab audio sharing over HTTPS.');
  ...
  if (!audioTracks.length) {
    ...
    throw new Error('No audio was shared. Choose a browser tab and enable Share tab audio, then try again.');
  }
  stopSilentDiscoBroadcast();
  state.silentDiscoCapture = capture;
  state.silentDiscoStream = new MediaStream(audioTracks);
  state.roomInternal.silentDiscoActive = true;
  const endHandler = () => { if (state.silentDiscoCapture === capture) stopSilentDiscoBroadcast(); };
  for (const track of capture.getTracks()) track.addEventListener?.('ended', endHandler, { once: true });
  roomChanged();
  for (const conn of state.guestConnections.values()) callSilentDiscoGuest(conn);
  showToast('Silent Disco broadcast started');
}

function collectionArt(item) {
  if (item.image) { const image = el('img'); image.src = item.image; image.alt = ''; image.loading = 'lazy'; return image; }
  return el('div', 'library-item-art', item.type === 'album' ? 'ALB' : 'LIST');
}

function renderCollectionList(collections = []) {
  state.libraryView = { kind: 'list', collections };
  refs.spotifyLibraryTitle.textContent = 'Playlists And Albums';
  refs.backToLibraryButton.classList.add('hidden');
  refs.spotifyLibraryTabs.classList.remove('hidden');
  refs.spotifyLibraryList.replaceChildren();
  if (!collections.length) return refs.spotifyLibraryList.append(el('div', 'empty-state', 'No saved collections found.'));
  for (const collection of collections) {
    const row = el('div', 'library-item');
    const copy = el('div', 'library-item-copy');
    copy.append(el('strong', '', collection.name), el('span', '', `${collection.owner || collection.description || 'Spotify'} · ${collection.trackCount || 0} tracks`));
    const actions = el('div', 'track-actions');
    const open = el('button', 'button small-button', 'View');
    open.type = 'button';
    open.addEventListener('click', () => loadCollection(collection.type, collection.id));
    actions.append(open);
    if (collection.externalUrl) {
      const spotifyLink = el('a', 'button small-button ghost', 'Open Spotify');
      spotifyLink.href = collection.externalUrl;
      spotifyLink.target = '_blank';
      spotifyLink.rel = 'noopener noreferrer';
      actions.append(spotifyLink);
    }
    row.append(collectionArt(collection), copy, actions);
    refs.spotifyLibraryList.append(row);
  }
}

function renderCollection(collection) {
  if (!collection) return;
  state.libraryView = { kind: 'collection', collection };
  refs.spotifyLibraryTitle.textContent = collection.name;
  refs.spotifyLibraryMessage.textContent = `${collection.owner || collection.description || 'Spotify'} · ${collection.trackCount || collection.tracks?.length || 0} tracks`;
  refs.spotifyLibraryTabs.classList.add('hidden');
  refs.backToLibraryButton.classList.toggle('hidden', !state.isHost);
  refs.spotifyLibraryList.replaceChildren();
  if (state.isHost) {
    const play = el('button', 'button primary full-width', `Play ${collection.type || 'collection'} on Spotify`);
    play.type = 'button';
    play.addEventListener('click', async () => { play.disabled = true; try { await playSpotifyCollection(collection); showToast(`Playing ${collection.name}`); } catch (error) { showToast(error.message); } finally { play.disabled = false; } });
    refs.spotifyLibraryList.append(play);
  }
  if (collection.externalUrl) {
    const spotifyLink = el('a', 'button ghost full-width', 'Open this collection in Spotify');
    spotifyLink.href = collection.externalUrl;
    spotifyLink.target = '_blank';
    spotifyLink.rel = 'noopener noreferrer';
    refs.spotifyLibraryList.append(spotifyLink);
  }
  for (const track of collection.tracks || []) {
    const actions = el('div', 'track-actions');
    if (state.isHost) {
      const add = el('button', 'button small-button primary', 'Add');
      add.type = 'button';
      add.addEventListener('click', async () => { add.disabled = true; try { await requestTrack(track.id); showToast(`${track.name} added`); } catch (error) { showToast(error.message); } finally { add.disabled = false; } });
      actions.append(add);
    }
    const row = el('div', 'library-item');
    row.append(createArt(track, 'library-item-art'), createTrackCopy(track), actions);
    refs.spotifyLibraryList.append(row);
  }
}
...
async function loadLibrary(type) {
  ...
  refs.spotifyLibraryMessage.textContent = `Loading ${type}…`;
  try { const collections = await listSpotifyCollections(type); refs.spotifyLibraryMessage.textContent = `${collections.length} ${type}`; renderCollectionList(collections); }
  catch (error) { refs.spotifyLibraryMessage.textContent = error.message; }
}

async function loadCollection(type, id) {
  if (!state.isHost) return;
  refs.spotifyLibraryMessage.textContent = 'Loading tracks…';
  try { const collection = await getSpotifyCollection(type, id); state.roomInternal.featuredCollection = collection; roomChanged(); renderCollection(collection); }
  catch (error) { refs.spotifyLibraryMessage.textContent = error.message; }
}
...
async function playNext(forcedItemId = '', fromAuto = false) {
  ...
  if (!item) {
    if (fromAuto && room.nowPlaying) {
      if (['spotify', 'host-listening', 'host-library'].includes(room.nowPlaying.source)) return false;
...
async function syncPlayback() {
  ...
  room.nowPlaying = {
    ...
    source: sameTrack ? room.nowPlaying?.source || 'host-listening' : 'host-listening',
    listeningContext: data.context ? {
      type: data.context.type || '',
      uri: data.context.uri || '',
      name: room.featuredCollection?.uri === data.context.uri ? room.featuredCollection.name : `Host ${data.context.type || 'Spotify'} playback`
    } : null
  };
...
function broadcastState() {
  ...
  for (const [clientId, conn] of state.guestConnections) {
    ...
    try { conn.send({ type: 'state', room: publicRoom(state.roomInternal, conn._jukeboxMember || { isHost: false }) }); } catch {}
  }
}
...
async function handleGuestCommand(conn, message) {
  ...
  if (message.type === 'join') {
    ...
    sendReply(conn, requestId, true, { room: publicRoom(state.roomInternal, conn._jukeboxMember) });
    maybeNotifyConnectedMember(conn._jukeboxMember);
    callSilentDiscoGuest(conn);
  ...
  try {
    if (message.type === 'syncRoom') {
      return sendReply(conn, requestId, true, { room: publicRoom(state.roomInternal, member) });
    }
    ...
    if (message.type === 'chat') {
      ...
      return sendReply(conn, requestId, true, { message: chatMessage, room: publicRoom(state.roomInternal, member) });
    }
...
function wireHostIncomingConnection(conn) {
  ...
  conn.on('close', () => {
    ...
    const mediaCall = state.silentDiscoCalls.get(conn.peer);
    try { mediaCall?.close(); } catch {}
    state.silentDiscoCalls.delete(conn.peer);
  });
}
...
async function createGuestPeer() {
  ...
  return new Promise((resolve, reject) => {
    ...
    peer.on('call', handleIncomingSilentDiscoCall);
    let settled = false;
...
function handleGuestPeerMessage(message) {
  ...
  if (message.type === 'screenEvent' && message.screenEvent) {
    showScreenEvent(message.screenEvent);
    return;
  }
...
function renderJukeboxView() {
  ...
  if (!active) {
    ...
    const hostListening = ['spotify', 'host-listening', 'host-library'].includes(active.source);
    refs.jukeboxPlaybackState.textContent = active.isPlaying ? (hostListening ? 'Listening With The Host' : 'Now Playing') : 'Paused';
    ...
    refs.jukeboxRequestedBy.textContent = hostListening
      ? `Host ${active.listeningContext?.type || 'Spotify'} · ${active.listeningContext?.name || 'Now playing'}`
      : active.requestedBy && active.requestedBy !== 'Spotify' ? `Requested by ${active.requestedBy}` : '';
...
function updatePipView() {
  ...
  if (!pipWindow || pipWindow.closed) { state.pipWindow = null; return; }
  const root = pipWindow.document.querySelector('#pipJukebox');
  if (!root) return;
  const playing = state.room?.nowPlaying;
  const visible = playing && !playbackEffectivelyEnded(playing) ? playing : null;
  const status = visible ? (visible.isPlaying ? 'Now Playing' : 'Paused') : 'Waiting For Music';
  const art = visible?.track?.image ? pipWindow.document.createElement('img') : pipWindow.document.createElement('div');
  if (visible?.track?.image) { art.src = visible.track.image; art.alt = ''; } else { art.className = 'pip-placeholder'; art.textContent = '♪'; }
  const copy = pipWindow.document.createElement('div');
  const statusNode = pipWindow.document.createElement('p'); statusNode.className = 'pip-status'; statusNode.textContent = status;
  const title = pipWindow.document.createElement('h1'); title.textContent = visible?.track?.name || 'Game Night Jukebox';
  const artist = pipWindow.document.createElement('p'); artist.textContent = visible?.track?.artists || 'The next song will appear here.';
  const context = pipWindow.document.createElement('p'); context.textContent = visible?.listeningContext?.name || (visible?.requestedBy && visible.requestedBy !== 'Spotify' ? `Requested by ${visible.requestedBy}` : '');
  copy.append(statusNode, title, artist, context);
  root.replaceChildren(art, copy);
}

async function openJukeboxPip() {
  if (!('documentPictureInPicture' in window) || typeof window.documentPictureInPicture.requestWindow !== 'function') return showToast('Chrome Document Picture-in-Picture is not available in this browser.');
  try {
    ...
    for (const sheet of document.styleSheets) {
      ...
      const link = pipWindow.document.createElement('link'); link.rel = 'stylesheet'; link.href = sheet.href; pipWindow.document.head.append(link);
    }
    pipWindow.document.body.innerHTML = '<main id="pipJukebox" class="pip-jukebox"></main>';
    pipWindow.addEventListener('pagehide', () => { state.pipWindow = null; });
    updatePipView();
  } catch (error) { showToast(error.message || 'Could not open Picture-in-Picture.'); }
}
...
function renderNowPlaying() {
  ...
  if (!playing) {
    ...
    refs.nowPlayingMeta.textContent = state.room?.hostListeningHidden ? 'Host Spotify playback is private.' : queueReady ? 'The queue is ready to start.' : 'Add a track to get started.';
  ...
  if (!ended && ['spotify', 'host-listening', 'host-library'].includes(playing.source)) pieces.push(`host ${playing.listeningContext?.type || 'Spotify'} playback`);
...
function updateProgress() {
  ...
  updatePipView();
  syncSpotifyForLiveDisplay();
}

async function syncSpotifyForLiveDisplay() {
  const liveDisplayOpen = Boolean(state.pipWindow && !state.pipWindow.closed) || state.viewMode === 'jukebox';
  if (!liveDisplayOpen || !state.isHost || !state.room?.spotifyConnected || state.room.mockMode || state.spotifyAutoSyncBusy) return;
  const now = Date.now();
  if (now - state.lastSpotifyAutoSyncAt < 12000) return;
  state.lastSpotifyAutoSyncAt = now;
  state.spotifyAutoSyncBusy = true;
  try { await syncPlayback(); }
  catch (error) { refs.hostMessage.textContent = `Live display sync: ${error.message}`; }
  finally { state.spotifyAutoSyncBusy = false; }
}
...
function renderRoom() {
  ...
  if (spotifyPlaylistAccessNeedsRefresh()) refs.spotifyBadge.textContent = 'Spotify access needs refresh';
  ...
  refs.spotifySessionActions.classList.toggle('hidden', !state.isHost || !state.room.spotifyConnected || state.room.mockMode);
  ...
  refs.showHostListeningInput.checked = state.room.showHostListening === true;
  refs.allowLibraryViewingInput.checked = state.room.allowLibraryViewing === true;
  ...
  const canViewLibrary = state.isHost || Boolean(state.room.featuredCollection && state.room.allowLibraryViewing);
  refs.spotifyLibraryPanel.classList.toggle('hidden', !canViewLibrary);
  refs.spotifyLibraryBadge.textContent = state.room.allowLibraryViewing ? 'Shared' : 'Private';
  refs.spotifyLibraryBadge.classList.toggle('active', state.room.allowLibraryViewing);
  if (!state.isHost && state.room.featuredCollection) renderCollection(state.room.featuredCollection);
  if (state.isHost && !state.libraryView && state.room.featuredCollection) renderCollection(state.room.featuredCollection);
  ...
  if (state.isHost) {
    ...
    refs.hostMessage.textContent = state.room.mockMode
      ...
      : state.room.spotifyConnected
        ? spotifyPlaylistAccessNeedsRefresh()
          ? `Refresh Spotify sign-in to grant playlist access. ${peerMode}.`
...
function updateHostSettings(settings) {
  ...
  if (typeof settings.showHostListening === 'boolean') room.showHostListening = settings.showHostListening;
  if (typeof settings.allowLibraryViewing === 'boolean') room.allowLibraryViewing = settings.allowLibraryViewing;
...
async function beginSpotifyLogin(forceDialog = false) {
  ...
  if (forceDialog) authorize.searchParams.set('show_dialog', 'true');
  location.href = authorize.toString();
}

function signOutSpotify() {
  localStorage.removeItem('jukebox.spotify.pkce.tokens');
  ...
  state.spotifySdkPromise = null;
  if (state.roomInternal) {
    if (state.roomInternal.nowPlaying && ['spotify', 'host-listening', 'host-library'].includes(state.roomInternal.nowPlaying.source)) archiveNowPlaying('spotify-signout');
    clearPlaybackTimer();
    state.roomInternal.selectedDeviceId = '';
    state.roomInternal.selectedDeviceName = '';
    roomChanged();
  }
  refs.browserPlayerStatus.textContent = 'Make this jukebox tab a Spotify Connect device.';
}
...
function leaveRoom() {
  ...
  if (wasHost) stopSilentDiscoBroadcast();
  else {
    const call = state.silentDiscoIncomingCall;
    clearSilentDiscoGuestAudio();
    try { call?.close(); } catch {}
  }
  clearTimeout(state.guestReconnectTimer);
  state.guestReconnectTimer = null;
  ...
  state.roomSyncBusy = false;
  state.lastSpotifyAutoSyncAt = 0;
  state.spotifyAutoSyncBusy = false;
  state.viewMode = 'room';
...
refs.openPipButton.addEventListener('click', openJukeboxPip);
refs.loadPlaylistsButton.addEventListener('click', () => loadLibrary('playlists'));
refs.loadAlbumsButton.addEventListener('click', () => loadLibrary('albums'));
refs.backToLibraryButton.addEventListener('click', () => loadLibrary('playlists'));
for (const button of refs.screenEventButtons) {
  button.addEventListener('click', () => {
    try { publishScreenEvent(button.dataset.screenEvent || ''); } catch (error) { showToast(error.message); }
  });
}
...
refs.refreshSpotifySigninButton.addEventListener('click', () => beginSpotifyLogin(true).catch((error) => showToast(error.message)));
refs.signOutSpotifyButton.addEventListener('click', () => {
  signOutSpotify();
  showToast('Spotify signed out from this jukebox');
});
refs.startSilentDiscoButton.addEventListener('click', () => {
  startSilentDiscoBroadcast().catch((error) => showToast(error.message));
});
refs.stopSilentDiscoButton.addEventListener('click', stopSilentDiscoBroadcast);
refs.joinSilentDiscoButton.addEventListener('click', async () => {
  if (!state.silentDiscoRemoteStream) return;
  try {
    refs.silentDiscoAudio.srcObject = state.silentDiscoRemoteStream;
    await refs.silentDiscoAudio.play();
    state.silentDiscoListening = true;
    renderSilentDiscoControls();
  } catch (error) {
    showToast(error.message || 'Tap Join again to enable host audio.');
  }
});
refs.leaveSilentDiscoButton.addEventListener('click', leaveSilentDiscoAudio);
...
refs.showHostListeningInput.addEventListener('change', () => hostPlayerAction('settings', { showHostListening: refs.showHostListeningInput.checked }));
refs.allowLibraryViewingInput.addEventListener('change', () => hostPlayerAction('settings', { allowLibraryViewing: refs.allowLibraryViewingInput.checked }));
...
window.addEventListener('beforeunload', () => {
  ...
  try { state.browserPlayer?.disconnect(); } catch {}
  for (const call of state.silentDiscoCalls.values()) { try { call.close(); } catch {} }
  for (const track of state.silentDiscoCapture?.getTracks?.() || []) { try { track.stop(); } catch {} }
  try { state.silentDiscoIncomingCall?.close(); } catch {}
  try { state.hostConnection?.close(); } catch {}
  try { state.peer?.destroy(); } catch {}
});
...
window.render_game_to_text = () => JSON.stringify({
  ...
  nowPlaying: state.room?.nowPlaying ? { title: state.room.nowPlaying.track?.name || '', artist: state.room.nowPlaying.track?.artists || '', playing: Boolean(state.room.nowPlaying.isPlaying), source: state.room.nowPlaying.source || '' } : null,
  queueCount: Number(state.room?.queueCount || 0),
  featuredCollection: state.room?.featuredCollection?.name || '',
  screenEvent: state.activeScreenEvent,
  silentDisco: {
    active: Boolean(state.room?.silentDiscoActive),
    streamReady: Boolean(state.silentDiscoRemoteStream),
    listening: Boolean(state.silentDiscoListening),
    listeners: state.silentDiscoCalls.size
  }
});
window.advanceTime = () => { updateProgress(); updatePipView(); };
