(() => {
  ...
  const state = {
    ...
    mediaCarrier: null, mediaStreamActive: false,
  ...
  const interactionSelectors = [
    ...
    '#mediaControlsBtn', '#startMediaStreamBtn', '#stopMediaStreamBtn', '#mediaPrevOfficeBtn', '#mediaNextOfficeBtn', '#mediaPauseResumeBtn', '#mediaPipBtn', '#openMediaControlsFromSettingsBtn',
  ...
  async function initBackendBridge() {
    ...
    try {
      ...
      const backendCompatible = !info.version || ['0.2.0','0.2.1'].includes(info.version);
  ...
  function cycleMediaOffice(direction = 1) {
    const directory = mediaOfficeDirectory();
    if (!directory.length) return;
    const currentId = activeMediaOfficeId();
    const currentIndex = Math.max(0, directory.findIndex(office => office.officeId === currentId));
    const nextIndex = (currentIndex + direction + directory.length) % directory.length;
    const next = directory[nextIndex];
    state.mediaMonitorOfficeId = next.officeId;
    updateMediaSession();
    updateMediaControlUi();
    renderPip();
    toast(`Monitoring ${next.officeName || next.officeId}.`);
    Debug.info('media-session', 'Media monitor office changed', { officeId: next.officeId, officeName: next.officeName || next.officeId });
  }
  ...
  function updateMediaControlUi() {
    ...
    const prev = $('mediaPrevOfficeBtn');
    const next = $('mediaNextOfficeBtn');
    ...
    if (prev) prev.disabled = !state.presence || mediaOfficeDirectory().length < 2;
    if (next) next.disabled = !state.presence || mediaOfficeDirectory().length < 2;
    if (pauseResume) {
  ...
  function updateMediaSession() {
    ...
    if ('MediaMetadata' in window) {
      try {
        navigator.mediaSession.metadata = new MediaMetadata({
          title: `${office} • ${status}`,
          artist: `Office Safety live monitoring • My status ${myStatus}`,
          album: `${peopleCount} ${peopleCount === 1 ? 'person' : 'people'} checked in`,
          artwork: [
    ...
    setHandler('previoustrack', () => cycleMediaOffice(-1));
    setHandler('nexttrack', () => cycleMediaOffice(1));
    setHandler('previousslide', () => cycleMediaOffice(-1));
    setHandler('nextslide', () => cycleMediaOffice(1));
  ...
  function bindEvents() {
    ...
    $('mediaPrevOfficeBtn').addEventListener('click', () => cycleMediaOffice(-1));
    $('mediaNextOfficeBtn').addEventListener('click', () => cycleMediaOffice(1));
