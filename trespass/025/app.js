(() => {
  ...
  function loadSettings() {
    try {
      ...
      return {
        ...
        quickControls: false, mediaControls: false, mediaStreamPreferred: false, mediaCarrierMode: 'audio', mediaVideoQuality: 'low', mediaVideoShowRoster: false
      };
    }
  }
  ...
  const interactionSelectors = [
    ...
    '#mediaControlsBtn', '#startMediaStreamBtn', '#stopMediaStreamBtn', '#mediaLowerAlertBtn', '#mediaRaiseAlertBtn', '#mediaRefreshStatusesBtn', '#mediaPipBtn', '#mediaCastBtn', '#mediaFullscreenBtn', '#openMediaControlsFromSettingsBtn',
  ...
  async function initBackendBridge() {
    ...
    try {
      ...
      const backendCompatible = !info.version || ['0.2.0','0.2.1','0.2.2','0.2.3','0.2.4','0.2.5'].includes(info.version);
  ...
  async function handleExternalAction(action, data = {}) {
    ...
    if (action === 'sync-now') {
      await runExclusive('Syncing Queued Actions', async () => {
        ...
        if (state.token) await performPoll();
  ...
  async function promptStatusVideoCast() {
    if (mediaCarrierMode() !== 'video' || !state.mediaVideoCarrier?.srcObject) {
      throw new Error('Start the status video stream before choosing a cast device.');
    }
    const remote = state.mediaVideoCarrier.remote;
    if (!remote?.prompt) {
      throw new Error('Direct remote playback is not exposed by this browser. Use Chrome Cast tab or your device screen-cast control while the TV view is open.');
    }
    try {
      await remote.prompt();
      state.mediaVideoRemoteState = remote.state || 'connecting';
      updateMediaControlUi();
      Debug.info('media-video', 'Remote playback device picker requested', { state: state.mediaVideoRemoteState });
    } catch (error) {
      Debug.warn('media-video', 'Remote playback prompt failed', { error });
      throw new Error('This browser could not cast the generated live stream directly. Use Chrome Cast tab or screen casting instead.');
    }
  }
  ...
  function updateMediaControlUi() {
    ...
    const directCast = Boolean(state.mediaVideoCarrier?.remote?.prompt);
    const values = {
      ...
      mediaRemotePlaybackState: mode === 'video' && active ? (directCast ? `Available • ${state.mediaVideoRemoteState}` : 'Use Chrome tab or screen casting') : 'Start status video first'
    };
    ...
    if (cast) cast.disabled = !(mode === 'video' && active && directCast);
  ...
  function bindEvents() {
    ...
    $('startMediaStreamBtn').addEventListener('click', () => {
      startPreferredMediaStream({ source: 'media-controls-dialog', resumeMonitoring: true })
        .then(() => toast(mediaCarrierMode() === 'video'
          ? 'Office status video started. Use Cast Device where supported or Chrome tab casting for a TV display.'
    ...
    $('mediaCastBtn').addEventListener('click', () => promptStatusVideoCast().catch(error => toast(error.message || 'Could not open cast controls.', 'error')));
        ...
        (async () => {
          try {
            ...
            if (task === 'periodic-refresh') {
              ...
              if (state.token && !state.uiLock && !state.pollInFlight) await performPoll();
              port?.postMessage({ ok:true, task, outbox, refreshed: Boolean(state.token) });
              return;
            }
