(() => {
  ...
  const GUEST_SESSION_TOKEN_KEY = 'officeSafetyGuestSessionToken';
  const QUICK_ACTION_PARAM = 'officeSafetyAction';
  const Background = window.OfficeSafetyBackground || null;
  const NativeCast = window.OfficeSafetyCast || null;
  const state = {
    token: '', guestToken: '', user: null, isAdmin: false, offices: [], presence: null, guestConfig: { enabled: true },
    ...
    mediaCarrier: null, mediaVideoCarrier: null, mediaVideoCanvas: null, mediaVideoStream: null, mediaVideoAudioContext: null, mediaVideoSilentSource: null, mediaVideoRemoteState: 'disconnected', mediaStreamActive: false, mediaLastKeepAliveAt: 0, mediaRefreshWatchdog: null, lastPollSuccessAt: 0, mediaMetadataFingerprint: '',
    nativeCastStatus: null, nativeCastHeartbeat: null,
  ...
  function loadSettings() {
    try {
      return {
        ...
        mediaCarrierMode: 'audio',
        mediaVideoQuality: 'low',
        mediaVideoShowRoster: false,
        castReceiverAppId: String(cfg.castReceiverAppId || ''),
      ...
      return {
        ...
        quickControls: false, mediaControls: false, mediaStreamPreferred: false, mediaCarrierMode: 'audio', mediaVideoQuality: 'low', mediaVideoShowRoster: false, castReceiverAppId: String(cfg.castReceiverAppId || '')
      };
    }
  }
  ...
  const interactionSelectors = [
    '#guestModeBtn', '#guestCancelBtn', '#guestLoginBtn', '#checkInBtn', '#updateStatusBtn', '#checkOutBtn', '#changeOfficeBtn', '#sendMessageBtn',
    '#refreshBtn', '#refreshStatusesBtn', '#adminRefreshBtn', '#adminSendMessageBtn', '#loadMoreMessagesBtn',
    ...
    '#mediaControlsBtn', '#startMediaStreamBtn', '#stopMediaStreamBtn', '#mediaLowerAlertBtn', '#mediaRaiseAlertBtn', '#mediaRefreshStatusesBtn', '#mediaPipBtn', '#mediaCastBtn', '#mediaCastStopBtn', '#mediaFullscreenBtn', '#openMediaControlsFromSettingsBtn',
  ...
  async function api(action, data = {}) {
    ...
    Debug.info('app-api', 'API call started', { action, dataKeys: data && typeof data === 'object' ? Object.keys(data) : [], signedIn: hasAuthSession(), authType: state.user?.authType || (state.guestToken ? 'guest' : (state.token ? 'google' : 'none')) });
    try {
  ...
  function hasAuthSession() {
    return Boolean(state.token || state.guestToken);
  }

  function isGuestSession() {
    return state.user?.authType === 'guest' || Boolean(state.guestToken && !state.token);
  }

  function displayEmailFor(value) {
    if (!value) return '';
    if (typeof value === 'string') return value.endsWith('@guest.invalid') ? '' : value;
    const explicit = String(value.displayEmail || value.authorDisplayEmail || value.subjectDisplayEmail || value.actorDisplayEmail || '');
    if (explicit) return explicit;
    const email = String(value.email || value.authorEmail || value.subjectEmail || value.actorEmail || '');
    return email.endsWith('@guest.invalid') ? '' : email;
  }

  function identityMeta(value) {
    const guest = String(value?.authType || '') === 'guest' || String(value?.email || value?.authorEmail || value?.actorEmail || '').endsWith('@guest.invalid');
    const email = displayEmailFor(value);
    if (guest) return email ? `${email} • Guest` : 'Guest';
    return email;
  }
  ...
  async function updateQueuedCount() {
    ...
    try {
      ...
      if (retryButton) retryButton.disabled = !state.queuedCount || !hasAuthSession() || state.outboxFlushInFlight;
  ...
  async function flushOutbox({ reason = 'foreground' } = {}) {
    if (!Background || state.outboxFlushInFlight || !hasAuthSession() || state.uiLock) return { sent: 0, remaining: state.queuedCount || 0 };
  ...
  function updateBackgroundSettingsUi() {
    ...
    const values = {
      ...
      mediaVideoSettingsCapability: statusVideoSupported() ? 'Available' : 'Unavailable',
    ...
    const videoModeOption = $('mediaCarrierModeSetting')?.querySelector('option[value="video"]');
    if (videoModeOption) videoModeOption.disabled = !statusVideoSupported();
  ...
  async function initBackendBridge() {
    ...
    const originHelp = `Front-end origin: ${location.origin}. Apps Script ALLOWED_FRONTEND_ORIGINS must contain ${location.origin}. Google sign-in also requires this origin in Google Cloud Authorized JavaScript origins. Approved Google accounts use the Staff allowlist while guest access is controlled separately by the backend.`;
    ...
    try {
      ...
      const backendCompatible = !info.version || ['0.2.0','0.2.1','0.2.2','0.2.3','0.2.4','0.2.5','0.2.6','0.2.8'].includes(info.version);
      const escalationBroadcastSupported = !info.version || ['0.2.6','0.2.8'].includes(info.version);
      state.bridgeReady = true;
      Debug.info('startup', 'Backend bridge connected', { frontendVersion: VERSION, backendVersion: info.version || '', bridgeOrigin: info.origin || '', versionMatch: !info.version || info.version === VERSION, backendCompatible, escalationBroadcastSupported });
      ...
      if (!escalationBroadcastSupported) {
        $('diagBridge').textContent += ' Update Apps Script to v0.2.6 or later for cross-office traffic-light escalation broadcasts.';
        Debug.warn('startup', 'Cross-office escalation broadcasts require backend v0.2.6 or later', { backendVersion: info.version || '' });
      }
  ...
  function saveGuestSessionToken(token) {
    try {
      if (token) sessionStorage.setItem(GUEST_SESSION_TOKEN_KEY, String(token));
      else sessionStorage.removeItem(GUEST_SESSION_TOKEN_KEY);
    } catch (error) {
      Debug.warn('session', 'Could not update guest session storage', { error });
    }
  }

  function loadGuestSessionToken() {
    try { return String(sessionStorage.getItem(GUEST_SESSION_TOKEN_KEY) || ''); }
    catch { return ''; }
  }

  function clearGuestSessionToken() { saveGuestSessionToken(''); }
  ...
  function applyBootstrapData(data, source = 'google') {
    ...
    if (state.presence?.officeId) enterOffice(state.presence);
      ...
      $('welcomeName').textContent = `Kia ora ${state.user.name || displayEmailFor(state.user) || 'Guest'}`;
      setView('setupView');
    }
    setConnection(true, isGuestSession() ? `Guest • ${state.user.name || 'Guest'}` : `Signed in as ${displayEmailFor(state.user) || state.user.name}`);
  ...
  async function restoreSessionFromStorage() {
    if (state.sessionRestoreAttempted || hasAuthSession() || !state.bridgeReady) return false;
    state.sessionRestoreAttempted = true;

    const guestToken = loadGuestSessionToken();
    if (guestToken) {
      state.guestToken = guestToken;
      try {
        const data = await runExclusive('Restoring Guest Session', () => api('bootstrap'), {
          detail: 'Restoring your guest office session after the page refresh.'
        });
        if (!data) return false;
        applyBootstrapData(data, 'guest-page-refresh');
        Debug.info('session', 'Guest session restored after page refresh', { hasPresence: Boolean(state.presence) });
        return true;
      } catch (error) {
        clearGuestSessionToken();
        state.guestToken = '';
        Debug.warn('session', 'Stored guest session could not be restored', { error });
      }
    }
  ...
  function setGuestFormOpen(open) {
    const form = $('guestLoginForm');
    const button = $('guestModeBtn');
    if (!form || !button) return;
    form.classList.toggle('hidden', !open);
    button.classList.toggle('hidden', open);
    if (open) setTimeout(() => $('guestDisplayName')?.focus(), 0);
  }

  async function loadGuestConfig() {
    try {
      const guestConfig = await api('guestConfig');
      state.guestConfig = guestConfig || { enabled:false };
      const enabled = Boolean(state.guestConfig.enabled);
      $('guestModeBtn')?.classList.toggle('hidden', !enabled);
      $('guestAvailability')?.classList.toggle('hidden', !enabled);
      if (!enabled) setGuestFormOpen(false);
      Debug.info('guest', 'Guest access configuration loaded', { enabled, sessionHours: state.guestConfig.sessionHours || null });
    } catch (error) {
      state.guestConfig = { enabled:false };
      $('guestModeBtn')?.classList.add('hidden');
      $('guestAvailability')?.classList.add('hidden');
      Debug.warn('guest', 'Guest access configuration could not be loaded', { error });
    }
  }

  async function guestLogin(event) {
    event?.preventDefault?.();
    const displayName = String($('guestDisplayName')?.value || '').trim();
    const displayEmail = String($('guestDisplayEmail')?.value || '').trim();
    if (displayName.length < 2) return toast('Enter a display name with at least two characters.', 'warn');
    try {
      const data = await runExclusive('Starting Guest Session', () => api('guestLogin', { displayName, displayEmail }), {
        detail: 'Creating your guest identity and loading the office list.'
      });
      if (!data?.guestToken) throw new Error('Guest session token was not returned by the backend.');
      clearSessionToken();
      state.token = '';
      state.guestToken = data.guestToken;
      saveGuestSessionToken(state.guestToken);
      applyBootstrapData(data, 'guest-sign-in');
      setGuestFormOpen(false);
      Debug.info('guest', 'Guest session started', { displayName: data.user?.name || displayName, displayEmailPresent: Boolean(data.user?.displayEmail) });
    } catch (error) {
      state.guestToken = '';
      clearGuestSessionToken();
      toast(error.message || 'Guest sign-in failed.', 'error');
      Debug.error('guest', 'Guest sign-in failed', { error });
    }
  }
  ...
  function enterOffice(presence) {
    const previousOfficeId = state.presence?.officeId || '';
    const nextOfficeId = presence?.officeId || '';
    const officeChanged = Boolean(previousOfficeId && nextOfficeId && previousOfficeId !== nextOfficeId);
    state.presence = presence;
    if (officeChanged || (state.snapshot?.officeId && state.snapshot.officeId !== nextOfficeId)) {
      state.snapshot = null;
      state.mediaMetadataFingerprint = '';
    }
    state.mediaMonitorOfficeId = nextOfficeId;
  ...
  async function updateStatus(statusOverride = '') {
    ...
    try {
      const completed = await runExclusive('Updating Status', async () => {
        ...
        syncPresenceState(data.presence, { source: 'status-update', updateSelectedStatus: true });
  ...
  async function checkOut({ changeOffice = false } = {}) {
    try {
      await runExclusive(changeOffice ? 'Changing Office' : 'Checking Out', async () => {
        ...
        stopRealtime();
        closePip();
        await stopMediaStream({ clearSession: true, remember: true });
        stopNativeCastSession({ notify: false });
        $('checkInNote').value = '';
  ...
  function syncPresenceState(presence, { source = 'presence-sync', updateSelectedStatus = false } = {}) {
    if (!presence) return;
    const previousOfficeId = state.presence?.officeId || '';
    const nextOfficeId = presence.officeId || previousOfficeId;
    const officeChanged = Boolean(previousOfficeId && nextOfficeId && previousOfficeId !== nextOfficeId);
    state.presence = presence;
    state.mediaMonitorOfficeId = nextOfficeId;
    if (updateSelectedStatus && presence.status) {
      state.selectedStatus = presence.status;
      applyStatusSelection(presence.status);
    }

    if (officeChanged) {
      state.snapshot = null;
      state.mediaMetadataFingerprint = '';
    } else if (state.snapshot?.officeId === nextOfficeId) {
      const email = String(presence.email || state.user?.email || '').toLowerCase();
      const people = (state.snapshot.people || []).map(person =>
        String(person.email || '').toLowerCase() === email ? { ...person, ...presence } : person
      );
      if (!people.some(person => String(person.email || '').toLowerCase() === email) && email) people.push(presence);
      state.snapshot = { ...state.snapshot, people, officeStatus: deriveOfficeStatus(people) };
      if (Array.isArray(state.snapshot.directory)) {
        state.snapshot.directory = state.snapshot.directory.map(office => {
          if (office.officeId !== nextOfficeId) return office;
          const officePeople = (office.people || []).map(person =>
            String(person.email || '').toLowerCase() === email ? { ...person, ...presence } : person
          );
          return { ...office, people: officePeople, officeStatus: deriveOfficeStatus(officePeople) };
        });
      }
    }
    ...
    Debug.debug('media-session', 'Presence synchronised into live media state', {
      source, officeId: nextOfficeId, personalStatus: presence.status || '', officeChanged
    });
  }
  ...
  function renderOffice(snapshot) {
    ...
    const ownEmail = String(state.user?.email || state.presence?.email || '').toLowerCase();
    const ownRosterEntry = people.find(person => String(person.email || '').toLowerCase() === ownEmail);
    if (ownRosterEntry && state.presence?.officeId === snapshot.officeId) {
      state.presence = { ...state.presence, ...ownRosterEntry };
      if (ownRosterEntry.status) {
        state.selectedStatus = ownRosterEntry.status;
        applyStatusSelection(ownRosterEntry.status);
      }
    }
    ...
    $('messageFeed').innerHTML = messages.length ? messages.map(messageRowHtml).join('') : 'No recent messages.';
  ...
  function messageLocationLabel(message) {
    const sourceId = String(message?.sourceOfficeId || '');
    if (message?.sourceOfficeName) return String(message.sourceOfficeName);
    if (sourceId) return officeById(sourceId)?.name || sourceId;
    if (message?.officeId === '*') return 'All offices';
    return officeById(message?.officeId)?.name || String(message?.officeId || '');
  }

  function messageSubjectLabel(message) {
    const name = String(message?.subjectName || message?.authorName || 'Staff member');
    const email = String(message?.subjectDisplayEmail || message?.authorDisplayEmail || displayEmailFor({ email:message?.subjectEmail || message?.authorEmail || '', authType:message?.authType }) || '');
    const guest = String(message?.authType || '') === 'guest' || String(message?.subjectEmail || message?.authorEmail || '').endsWith('@guest.invalid');
    const suffix = guest ? (email ? `${email} • Guest` : 'Guest') : email;
    return suffix && suffix.toLowerCase() !== name.toLowerCase() ? `${name} (${suffix})` : name;
  }

  function isStatusEscalationMessage(message) {
    return String(message?.kind || '') === 'status_escalation';
  }

  function messageRowHtml(message) {
    const severity = String(message?.severity || 'green').toLowerCase();
    const escalation = isStatusEscalationMessage(message);
    const location = messageLocationLabel(message);
    const classes = ['message-row'];
    if (severity !== 'green') classes.push('important', severity);
    if (escalation) classes.push('escalation');
    if (escalation) {
      const subject = messageSubjectLabel(message);
      const previous = String(message.previousStatus || '').toUpperCase();
      const next = String(message.newStatus || severity).toUpperCase();
      const transition = previous ? `${previous} → ${next}` : next;
      const note = String(message.statusNote || '');
      return `<div class="${classes.map(escapeHtml).join(' ')}">
        <div class="message-meta"><strong>${escapeHtml(subject)}</strong> • ${escapeHtml(location || 'Unknown office')} • ${escapeHtml(next)} escalation • ${formatTime(message.createdAt)} • All offices</div>
        <p><strong>${escapeHtml(transition)}</strong>${note ? ` • ${escapeHtml(note)}` : ''}</p>
      </div>`;
    }
    const author = message.authorName || displayEmailFor({ email:message.authorEmail, displayEmail:message.authorDisplayEmail, authType:message.authType }) || 'Office Safety';
    const authorMeta = String(message.authType || '') === 'guest' ? (message.authorDisplayEmail ? ` • ${escapeHtml(message.authorDisplayEmail)} • Guest` : ' • Guest') : '';
    const audience = message.officeId === '*' ? 'All offices' : location;
    return `<div class="${classes.map(escapeHtml).join(' ')}">
      <div class="message-meta">${escapeHtml(author)}${authorMeta}${audience ? ` • ${escapeHtml(audience)}` : ''} • ${formatTime(message.createdAt)}</div>
      <p>${escapeHtml(message.text)}</p>
    </div>`;
  }

  function personRowHtml(person) {
    const meta = identityMeta(person);
    const guestClass = String(person.authType || '') === 'guest' ? ' guest-person' : '';
    return `<div class="person-row ${escapeHtml(person.status || 'green')}${guestClass}">
      <span class="person-status"></span>
      <div class="person-main"><strong>${escapeHtml(person.name || displayEmailFor(person) || 'Guest')}</strong><small>${meta ? `${escapeHtml(meta)} • ` : ''}${ago(person.updatedAt)}</small>${person.note ? `<div class="person-note">${escapeHtml(person.note)}</div>` : ''}</div>
  ...
  function notifyStatusChanges(people) {
    // Direct roster diffs are used only to maintain a baseline. Cross-office safety
    // notifications are driven by persisted status_escalation messages from the backend
    // so every office sees one authoritative event with staff and location context.
    const seen = new Set();
    for (const person of people) {
      const key = `${String(person.officeId || '')}|${String(person.email || person.name || '')}`;
      seen.add(key);
      const previous = state.lastPersonStates.get(key);
      const current = person.status || 'green';
      state.lastPersonStates.set(key, current);
      if (previous && previous !== current) {
        Debug.info('status-baseline', 'Roster status changed; waiting for authoritative escalation event if applicable', {
          officeId: person.officeId || '', staff: person.name || person.email || '', previousStatus: previous, currentStatus: current
        });
      }
    }
  ...
  function notifyNewMessages(messages) {
    ...
    for (const message of messages) {
      ...
      if (important) {
        ...
        updateAppBadge();
        if (escalation) {
          const location = messageLocationLabel(message) || 'Unknown office';
          const subject = message.subjectName || message.authorName || message.authorEmail || 'Staff member';
          toast(`${String(message.severity || '').toUpperCase()} escalation • ${location} • ${subject}`, message.severity === 'red' ? 'error' : 'warn');
        }
  ...
  async function showSystemNotification(message, { force = false } = {}) {
    ...
    const transition = previous ? `${previous} → ${next}` : next;
    const title = force
      ? 'Office Safety notification test'
      : escalation
        ? `${severity.toUpperCase()} escalation • ${location || 'Unknown office'}`
        : `${severity.toUpperCase()} office message${location ? ` • ${location}` : ''}`;
    const body = force
      ? 'System notifications are working for Office Safety.'
      : escalation
        ? `${subject} • ${transition}${message.statusNote ? ` • ${message.statusNote}` : ''}`
        : `${message.authorName || message.authorEmail || 'Office Safety'}: ${message.text || ''}`;
    const notificationOfficeId = escalation ? (message.sourceOfficeId || '') : (message.officeId || state.presence?.officeId || '');
    const options = {
      body,
      icon: './icon-192.png',
      badge: './badge-96.png',
      tag: force ? 'office-safety-test' : escalation ? `office-escalation-${message.id || Date.now()}` : `office-message-${message.id || Date.now()}`,
      renotify: !force,
      requireInteraction: severity === 'red' && !force,
      data: {
        url: './', officeId: notificationOfficeId, sourceOfficeId: message.sourceOfficeId || '', sourceOfficeName: location || '',
        subjectName: message.subjectName || '', subjectEmail: message.subjectEmail || '', messageId: message.id || '', severity, kind: message.kind || 'message'
      },
  ...
  async function handleExternalAction(action, data = {}) {
    ...
    Debug.info('external-action', 'Handling external action', { action, signedIn: hasAuthSession(), hasPresence: Boolean(state.presence) });
    if (!hasAuthSession() || state.uiLock) {
      state.pendingExternalAction = { action, data };
      return;
    }
    ...
    if (action === 'sync-now') {
      await runExclusive('Syncing Queued Actions', async () => {
        ...
        if (hasAuthSession()) await performPoll();
    ...
    if (action === 'media-video') {
      state.settings.mediaCarrierMode = 'video';
      saveSettings();
      openMediaControls();
      return;
    }
  ...
  function consumePendingExternalAction() {
    if (!state.pendingExternalAction || !hasAuthSession() || state.uiLock) return;
  ...
  function activeMediaOfficeId() {
    // Media controls always represent the office the signed-in person is actually checked into.
    // Previous/Next are alert controls now, so retaining an older monitored office would make
    // the lock-screen card drift away from the authoritative My Office state.
    return state.presence?.officeId || '';
  }
  ...
  const MEDIA_VIDEO_QUALITIES = {
    low: { width: 854, height: 480, fps: 1, label: 'Low bandwidth 854×480 • 1 fps' },
    standard: { width: 1280, height: 720, fps: 1, label: 'TV 1280×720 • 1 fps' }
  };

  function mediaCarrierMode() {
    return state.settings.mediaCarrierMode === 'video' ? 'video' : 'audio';
  }

  function statusVideoSupported() {
    return Boolean(window.HTMLCanvasElement?.prototype?.captureStream && window.MediaStream);
  }

  function currentMediaElement() {
    if (mediaCarrierMode() === 'video' && state.mediaVideoCarrier) return state.mediaVideoCarrier;
    return state.mediaCarrier || state.mediaVideoCarrier || null;
  }
  ...
  function ensureMediaCarrier() {
    ...
    audio.tabIndex = -1;
    const syncState = () => {
      state.mediaStreamActive = mediaCarrierIsPlaying();
      updateMediaControlUi();
      updateMediaSession();
    };
    audio.addEventListener('play', syncState);
    audio.addEventListener('playing', () => { syncState(); mediaKeepAliveTick('audio-playing'); startMediaRefreshWatchdog(); });
    audio.addEventListener('timeupdate', () => mediaKeepAliveTick('audio-timeupdate'));
    audio.addEventListener('pause', () => { stopMediaRefreshWatchdog(); syncState(); if (state.presence) startWorker(); });
  ...
  function videoQualityConfig() {
    return MEDIA_VIDEO_QUALITIES[state.settings.mediaVideoQuality] || MEDIA_VIDEO_QUALITIES.low;
  }

  function ensureStatusVideoCanvas() {
    if (!state.mediaVideoCanvas) {
      state.mediaVideoCanvas = document.createElement('canvas');
      state.mediaVideoCanvas.id = 'officeSafetyStatusVideoCanvas';
    }
    const quality = videoQualityConfig();
    if (state.mediaVideoCanvas.width !== quality.width) state.mediaVideoCanvas.width = quality.width;
    if (state.mediaVideoCanvas.height !== quality.height) state.mediaVideoCanvas.height = quality.height;
    return state.mediaVideoCanvas;
  }

  function renderStatusVideoFrame(reason = 'state-sync') {
    if (!state.presence || !statusVideoSupported()) return;
    const canvas = ensureStatusVideoCanvas();
    const ctx = canvas.getContext('2d', { alpha: false });
    if (!ctx) return;
    const snapshot = currentPipSnapshot();
    const officeId = state.presence.officeId || snapshot?.officeId || '';
    const office = snapshot?.officeName || officeById(officeId)?.name || officeId || 'Office';
    const people = Array.isArray(snapshot?.people) ? snapshot.people : [];
    const status = String(snapshot?.officeStatus || deriveOfficeStatus(people) || 'green').toLowerCase();
    const myStatus = String(state.presence.status || state.selectedStatus || 'green').toLowerCase();
    const palette = {
      green: '#138a4a', amber: '#b97800', red: '#b42318', empty: '#64748b'
    };
    const statusColour = palette[status] || palette.green;
    const width = canvas.width;
    const height = canvas.height;
    const scale = width / 854;
    ...
    ctx.fillRect(0, 0, width, px(72));
    ctx.fillStyle = '#f8fafc';
    ctx.font = `700 ${px(22)}px system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif`;
    ctx.fillText('OFFICE SAFETY', pad, px(45));
    ctx.textAlign = 'right';
    ctx.font = `500 ${px(15)}px system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif`;
    ctx.fillStyle = '#cbd5e1';
    ctx.fillText(`Live status • ${new Date().toLocaleTimeString('en-NZ', { hour: '2-digit', minute: '2-digit' })}`, width - pad, px(44));
    ctx.textAlign = 'left';

    ctx.fillStyle = '#f8fafc';
    ctx.font = `700 ${px(34)}px system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif`;
    ctx.fillText(office, pad, px(126));

    const centreX = px(190);
    const centreY = px(252);
    const radius = px(92);
    ctx.beginPath();
    ...
    ctx.font = `800 ${px(32)}px system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif`;
    ctx.fillText(status.toUpperCase(), centreX, centreY);
    ctx.textAlign = 'left';
    ctx.textBaseline = 'alphabetic';

    const infoX = px(330);
    ctx.fillStyle = '#94a3b8';
    ctx.font = `600 ${px(16)}px system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif`;
    ctx.fillText('CHECKED IN', infoX, px(198));
    ctx.fillStyle = '#f8fafc';
    ctx.font = `800 ${px(44)}px system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif`;
    ctx.fillText(String(people.length), infoX, px(244));
    ctx.fillStyle = '#94a3b8';
    ctx.font = `600 ${px(16)}px system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif`;
    ctx.fillText('MY STATUS', infoX, px(292));
    ctx.fillStyle = palette[myStatus] || palette.green;
    ctx.font = `800 ${px(30)}px system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif`;
    ctx.fillText(myStatus.toUpperCase(), infoX, px(330));

    if (state.settings.mediaVideoShowRoster && people.length) {
      const rosterX = px(570);
      ctx.fillStyle = '#94a3b8';
      ctx.font = `600 ${px(15)}px system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif`;
      ctx.fillText('LIVE ROSTER', rosterX, px(176));
      ctx.font = `600 ${px(16)}px system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif`;
      people.slice(0, 6).forEach((person, index) => {
        const y = px(211 + index * 35);
        const personStatus = String(person.status || 'green').toLowerCase();
        ctx.fillStyle = palette[personStatus] || palette.green;
        ctx.beginPath();
        ctx.arc(rosterX + px(7), y - px(5), px(7), 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = '#e2e8f0';
        const name = String(person.name || person.email || 'Staff member');
        const clipped = name.length > 24 ? `${name.slice(0, 23)}…` : name;
        ctx.fillText(clipped, rosterX + px(26), y);
      });
      if (people.length > 6) {
        ctx.fillStyle = '#94a3b8';
        ctx.font = `500 ${px(14)}px system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif`;
        ctx.fillText(`+ ${people.length - 6} more`, rosterX + px(26), px(421));
      }
    } else {
      ctx.fillStyle = '#94a3b8';
      ctx.font = `500 ${px(17)}px system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif`;
      ctx.fillText('Office traffic-light status display', infoX, px(386));
      ctx.fillText('Updates automatically with the live office state', infoX, px(414));
    }

    ctx.fillStyle = '#334155';
    ctx.fillRect(0, height - px(54), width, px(54));
    ctx.fillStyle = '#cbd5e1';
    ctx.font = `500 ${px(14)}px system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif`;
    ctx.fillText('Office Safety Traffic Light • supplementary coordination display', pad, height - px(21));

    const track = state.mediaVideoStream?.getVideoTracks?.()[0];
    try { track?.requestFrame?.(); } catch {}
    Debug.debug('media-video', 'Status video frame rendered', { reason, officeId, office, status, myStatus, peopleCount: people.length, width, height });
  }

  function ensureStatusVideoCarrier() {
    const video = $('statusVideoPreview') || document.createElement('video');
    if (!video.id) {
      video.id = 'statusVideoPreview';
      video.className = 'status-video-preview';
    ...
    video.muted = false;
    if (!video.dataset.officeSafetyBound) {
      video.dataset.officeSafetyBound = '1';
      const syncState = () => {
        state.mediaStreamActive = mediaCarrierIsPlaying();
        updateMediaControlUi();
        updateMediaSession();
      ...
      video.addEventListener('playing', () => { syncState(); mediaKeepAliveTick('video-playing'); startMediaRefreshWatchdog(); });
      video.addEventListener('timeupdate', () => mediaKeepAliveTick('video-timeupdate'));
      video.addEventListener('pause', () => { stopMediaRefreshWatchdog(); syncState(); if (state.presence) startWorker(); });
      video.addEventListener('ended', syncState);
      video.addEventListener('error', () => {
        Debug.warn('media-video', 'Status video media element failed', { mediaErrorCode: video.error?.code || null });
        updateMediaControlUi();
      });
      if (video.remote) {
        video.remote.addEventListener?.('connecting', () => { state.mediaVideoRemoteState = 'connecting'; updateMediaControlUi(); });
        video.remote.addEventListener?.('connect', () => { state.mediaVideoRemoteState = 'connected'; updateMediaControlUi(); });
        video.remote.addEventListener?.('disconnect', () => { state.mediaVideoRemoteState = 'disconnected'; updateMediaControlUi(); });
      }
    }
    state.mediaVideoCarrier = video;
    return video;
  }

  async function makeSilentVideoAudioTrack() {
    ...
    try {
      ...
      state.mediaVideoAudioContext = context;
      state.mediaVideoSilentSource = source;
      return destination.stream.getAudioTracks()[0] || null;
    } catch (error) {
      Debug.debug('media-video', 'Could not add silent audio track to status video stream', { error });
      return null;
    }
  }

  function stopAudioCarrierOnly() {
  ...
  async function stopStatusVideoCarrierOnly() {
    const video = state.mediaVideoCarrier;
    if (video) {
      try { video.pause(); } catch {}
      try { video.srcObject = null; } catch {}
    }
    for (const track of state.mediaVideoStream?.getTracks?.() || []) {
      try { track.stop(); } catch {}
    }
    state.mediaVideoStream = null;
    try { state.mediaVideoSilentSource?.stop?.(); } catch {}
    state.mediaVideoSilentSource = null;
    if (state.mediaVideoAudioContext) {
      try { await state.mediaVideoAudioContext.close(); } catch {}
    }
    state.mediaVideoAudioContext = null;
    state.mediaVideoRemoteState = 'disconnected';
  }

  function mediaCarrierIsPlaying() {
    const audioPlaying = Boolean(state.mediaCarrier && !state.mediaCarrier.paused && !state.mediaCarrier.ended);
    const videoPlaying = Boolean(state.mediaVideoCarrier && !state.mediaVideoCarrier.paused && !state.mediaVideoCarrier.ended && state.mediaVideoCarrier.srcObject);
    return audioPlaying || videoPlaying;
  }

  function mediaKeepAliveTick(source = 'media-timeupdate') {
    if (!state.presence || !mediaCarrierIsPlaying()) return;
    // Media playback must never suppress the normal office refresh loop. The media path is
    // a second refresh signal only, not the owner of polling state.
    if (state.liveMonitoringPaused) state.liveMonitoringPaused = false;
    startWorker();
    if (!state.heartbeat) startHeartbeat();
    const now = Date.now();
    const refreshEvery = Math.max(5000, Number(state.settings.pollMs || cfg.defaultPollMs || 10000));
    const staleFor = now - Number(state.lastPollSuccessAt || 0);
    if (now - Number(state.mediaLastKeepAliveAt || 0) < refreshEvery && staleFor < refreshEvery * 1.5) return;
    state.mediaLastKeepAliveAt = now;
    pollNow();
    Debug.debug('media-session', 'Media carrier keep-alive refresh tick', { source, refreshEvery, staleFor, mode: mediaCarrierMode() });
  }

  function stopMediaRefreshWatchdog() {
    if (state.mediaRefreshWatchdog) clearInterval(state.mediaRefreshWatchdog);
    state.mediaRefreshWatchdog = null;
  }

  function startMediaRefreshWatchdog() {
    stopMediaRefreshWatchdog();
    if (!state.presence || !mediaCarrierIsPlaying()) return;
    const interval = Math.max(4000, Math.min(10000, Number(state.settings.pollMs || cfg.defaultPollMs || 10000)));
    state.mediaRefreshWatchdog = setInterval(() => {
      if (!state.presence || !mediaCarrierIsPlaying()) return;
      const refreshEvery = Math.max(5000, Number(state.settings.pollMs || cfg.defaultPollMs || 10000));
      const staleFor = Date.now() - Number(state.lastPollSuccessAt || 0);
      // The Worker remains the primary scheduler. This only recovers if it has not produced
      // a successful snapshot in time while a media carrier is active.
      if (!state.pollInFlight && !state.uiLock && staleFor >= refreshEvery * 1.5) {
        Debug.warn('media-session', 'Media refresh watchdog detected stale office data', { staleFor, refreshEvery, mode: mediaCarrierMode() });
        startWorker();
        pollNow();
      }
    }, interval);
    Debug.debug('media-session', 'Media refresh watchdog started', { interval, mode: mediaCarrierMode() });
  }
  ...
  async function startSilentMediaStream({ source = 'ui', resumeMonitoring = true } = {}) {
    ...
    if (resumeMonitoring) {
      ...
      mediaKeepAliveTick('audio-start');
      startMediaRefreshWatchdog();
    }
    updateMediaSession();
    updateMediaControlUi();
    updateMediaControlsAvailability();
  ...
  async function startStatusVideoStream({ source = 'ui', resumeMonitoring = true } = {}) {
    if (!state.presence) throw new Error('Check into an office before starting the status video stream.');
    if (!('mediaSession' in navigator)) throw new Error('Media Session is not supported by this browser.');
    if (!statusVideoSupported()) throw new Error('Canvas video capture is not supported by this browser.');
    stopAudioCarrierOnly();
    await stopStatusVideoCarrierOnly();
    state.settings.mediaControls = true;
    state.settings.mediaStreamPreferred = true;
    state.settings.mediaCarrierMode = 'video';
    ...
    const canvas = ensureStatusVideoCanvas();
    renderStatusVideoFrame('video-start');
    const quality = videoQualityConfig();
    const stream = canvas.captureStream(quality.fps);
    const videoTrack = stream.getVideoTracks?.()[0];
    if (videoTrack) {
      try { videoTrack.contentHint = 'detail'; } catch {}
      try { await videoTrack.applyConstraints?.({ frameRate: { max: quality.fps } }); } catch {}
    }
    const silentTrack = await makeSilentVideoAudioTrack();
    if (silentTrack) stream.addTrack(silentTrack);
    state.mediaVideoStream = stream;
    const video = ensureStatusVideoCarrier();
    video.srcObject = stream;
    try {
      await video.play();
    } catch (error) {
      Debug.warn('media-video', 'Browser refused to start status video playback', { source, error });
      await stopStatusVideoCarrierOnly();
      throw new Error('The browser did not allow the status video stream to start. Tap Start Status Video directly from the page and try again.');
    }
    state.mediaStreamActive = true;
    if (resumeMonitoring) {
      state.liveMonitoringPaused = false;
      ...
      startHeartbeat();
      state.mediaLastKeepAliveAt = 0;
      mediaKeepAliveTick('video-start');
      startMediaRefreshWatchdog();
    }
    updateMediaSession();
    updateMediaControlUi();
    updateMediaControlsAvailability();
    Debug.info('media-video', 'Low-frame-rate office status video started', {
      source, officeId: activeMediaOfficeId(), width: quality.width, height: quality.height, fps: quality.fps, silentAudioTrack: Boolean(silentTrack)
    });
    return true;
  }

  async function startPreferredMediaStream({ source = 'ui', resumeMonitoring = true } = {}) {
    return mediaCarrierMode() === 'video'
      ? startStatusVideoStream({ source, resumeMonitoring })
      : startSilentMediaStream({ source, resumeMonitoring });
  }

  function pauseMediaStream({ pauseMonitoring = false, notify = false } = {}) {
    const element = currentMediaElement();
    if (element && !element.paused) element.pause();
    state.mediaStreamActive = false;
    stopMediaRefreshWatchdog();
    // Pausing media only pauses the carrier. Office polling and presence heartbeat continue.
    if (state.liveMonitoringPaused) state.liveMonitoringPaused = false;
    startWorker();
    if (!state.heartbeat) startHeartbeat();
    updateMediaSession();
    updateMediaControlUi();
    if (notify) toast('Media paused. Office statuses will keep refreshing.');
  }

  async function stopMediaStream({ clearSession = true, remember = false, notify = false } = {}) {
    stopAudioCarrierOnly();
    await stopStatusVideoCarrierOnly();
    state.mediaStreamActive = false;
    state.mediaLastKeepAliveAt = 0;
    ...
    updateMediaControlUi();
    updateMediaControlsAvailability();
    if (notify) toast('Media stream stopped.');
  }

  // Backwards-compatible wrappers retained for existing call paths and tests.
  function pauseSilentMediaStream(options = {}) { pauseMediaStream(options); }
  function stopSilentMediaStream(options = {}) { return stopMediaStream(options); }

  async function openStatusVideoFullscreen() {
    if (mediaCarrierMode() !== 'video' || !state.mediaVideoCarrier?.srcObject) {
      throw new Error('Start the status video stream first.');
    }
    const video = state.mediaVideoCarrier;
    ...
    throw new Error('Full-screen video is not available in this browser.');
  }

  function configuredCastReceiverAppId() {
    return String(state.settings.castReceiverAppId || cfg.castReceiverAppId || '').trim();
  }

  function nativeCastHttpsReady() {
    return location.protocol === 'https:' || ['localhost','127.0.0.1','[::1]'].includes(location.hostname);
  }

  function latestEscalationForCast(snapshot) {
    const messages = Array.isArray(snapshot?.messages) ? snapshot.messages : [];
    const escalation = messages.find(message => isStatusEscalationMessage(message));
    if (!escalation) return null;
    const created = new Date(escalation.createdAt || 0).getTime();
    if (!Number.isFinite(created) || Date.now() - created > 30 * 60 * 1000) return null;
    return {
      severity: String(escalation.severity || escalation.newStatus || 'amber').toLowerCase(),
      staffName: String(escalation.subjectName || escalation.authorName || 'Staff member'),
      officeName: messageLocationLabel(escalation),
      previousStatus: String(escalation.previousStatus || ''),
      newStatus: String(escalation.newStatus || escalation.severity || ''),
      note: String(escalation.statusNote || ''),
      createdAt: String(escalation.createdAt || '')
    };
  }

  function buildNativeCastPayload() {
    if (!state.presence) return null;
    const snapshot = currentPipSnapshot();
    const officeId = state.presence.officeId || snapshot?.officeId || '';
    const people = Array.isArray(snapshot?.people) ? snapshot.people : [];
    const officeStatus = String(snapshot?.officeStatus || deriveOfficeStatus(people) || 'empty').toLowerCase();
    return {
      type: 'office-safety-state',
      schemaVersion: 1,
      appVersion: VERSION,
      sentAt: new Date().toISOString(),
      office: {
        id: officeId,
        name: snapshot?.officeName || officeById(officeId)?.name || officeId || 'Office',
        status: officeStatus,
        peopleCount: people.length
      },
      me: {
        name: String(state.user?.name || state.presence?.name || 'Staff member'),
        status: String(state.presence.status || state.selectedStatus || 'green').toLowerCase()
      },
      roster: state.settings.mediaVideoShowRoster
        ? people.slice(0, 10).map(person => ({ name: String(person.name || person.email || 'Staff member'), status: String(person.status || 'green').toLowerCase() }))
        : [],
      latestEscalation: latestEscalationForCast(snapshot)
    };
  }

  function nativeCastStateText(status = state.nativeCastStatus) {
    const appId = configuredCastReceiverAppId();
    if (!NativeCast) return 'Unavailable in this build';
    if (!appId) return 'Custom receiver not configured';
    if (!nativeCastHttpsReady()) return 'HTTPS required';
    if (!status?.ready) return 'Cast SDK not initialised';
    if (status.connected) return `Connected${status.deviceName ? ` • ${status.deviceName}` : ''}`;
    const castState = String(status.castState || '');
    if (castState.includes('NO_DEVICES')) return 'No compatible Cast device found';
    if (castState.includes('CONNECTING')) return 'Connecting…';
    if (castState.includes('CONNECTED')) return 'Connected';
    return 'Ready for Cast';
  }

  function stopNativeCastHeartbeat() {
    if (state.nativeCastHeartbeat) clearInterval(state.nativeCastHeartbeat);
    state.nativeCastHeartbeat = null;
  }

  function startNativeCastHeartbeat() {
    stopNativeCastHeartbeat();
    if (!state.nativeCastStatus?.connected) return;
    state.nativeCastHeartbeat = setInterval(() => {
      syncNativeCastState('cast-heartbeat', { force: true }).catch(error => Debug.debug('cast', 'Cast heartbeat state send failed', { error }));
    }, 20000);
  }

  async function initialiseNativeCast({ silent = true } = {}) {
    const appId = configuredCastReceiverAppId();
    if (!NativeCast || !appId || !nativeCastHttpsReady()) {
      state.nativeCastStatus = NativeCast?.status?.() || null;
      updateMediaControlUi();
      return false;
    }
    try {
      state.nativeCastStatus = await NativeCast.configure({
        appId,
        debug: Debug,
        onStateChange(status) {
          state.nativeCastStatus = status;
          if (status.connected) startNativeCastHeartbeat(); else stopNativeCastHeartbeat();
          updateMediaControlUi();
        }
      });
      updateMediaControlUi();
      ...
      state.nativeCastStatus = { ready: false, configured: true, error: String(error?.message || error) };
      updateMediaControlUi();
      if (!silent) throw error;
      Debug.warn('cast', 'Native Cast initialisation failed', { error });
      return false;
    }
  }

  async function syncNativeCastState(reason = 'state-sync', { force = false } = {}) {
    const payload = buildNativeCastPayload();
    if (!NativeCast || !payload) return false;
    try {
      const sent = await NativeCast.sync(payload, { force, reason });
      state.nativeCastStatus = NativeCast.status();
      updateMediaControlUi();
      return sent;
    } catch (error) {
      Debug.warn('cast', 'Could not synchronise office state to Cast receiver', { reason, error });
      return false;
    }
  }

  async function promptStatusVideoCast() {
    const appId = configuredCastReceiverAppId();
    if (!NativeCast) throw new Error('Native Google Cast support is not included in this browser build.');
    if (!appId) throw new Error('Native Cast needs a Custom Web Receiver app ID. Register the included cast-receiver.html and enter its app ID in Settings.');
    if (!nativeCastHttpsReady()) throw new Error('Native Cast requires Office Safety to be served over HTTPS. localhost can be used for development.');
    await initialiseNativeCast({ silent: false });
    state.nativeCastStatus = await NativeCast.requestSession({ appId, debug: Debug, onStateChange(status) {
      state.nativeCastStatus = status;
      if (status.connected) startNativeCastHeartbeat(); else stopNativeCastHeartbeat();
      updateMediaControlUi();
    }});
    await syncNativeCastState('cast-session-start', { force: true });
    startNativeCastHeartbeat();
    updateMediaControlUi();
    Debug.info('cast', 'Native Office Safety Cast session started', { deviceName: state.nativeCastStatus?.deviceName || '', appId });
  }

  function stopNativeCastSession({ notify = true } = {}) {
    stopNativeCastHeartbeat();
    const stopped = NativeCast?.disconnect?.(true) || false;
    state.nativeCastStatus = NativeCast?.status?.() || null;
    updateMediaControlUi();
    if (notify && stopped) toast('Casting stopped.');
    return stopped;
  }
  ...
  function updateMediaControlsAvailability() {
    ...
    button.title = !supported
      ...
      : mediaCarrierIsPlaying()
        ? `Media controls are active using ${mediaCarrierMode() === 'video' ? 'the office status video' : 'silent audio'}`
  ...
  function updateMediaControlUi() {
    ...
    const videoSupport = statusVideoSupported();
    const castAppId = configuredCastReceiverAppId();
    const castStatus = state.nativeCastStatus || NativeCast?.status?.() || null;
    const values = {
      mediaControlSupportState: supported ? 'Available' : 'Unavailable',
      mediaStreamState: active ? (mode === 'video' ? 'Status video running' : 'Silent audio running') : 'Stopped',
      mediaMonitoredOffice: office,
      mediaMonitoringState: state.liveMonitoringPaused ? 'Live refresh paused' : 'Live refresh running',
      mediaVideoCapability: videoSupport ? 'Available' : 'Unavailable',
      mediaNativeCastState: nativeCastStateText(castStatus),
      nativeCastSettingsCapability: nativeCastStateText(castStatus)
    };
    ...
    const refreshStatuses = $('mediaRefreshStatusesBtn');
    const pip = $('mediaPipBtn');
    const cast = $('mediaCastBtn');
    const castStop = $('mediaCastStopBtn');
    const castLauncher = $('nativeCastLauncher');
    const fullscreen = $('mediaFullscreenBtn');
    const modeControl = $('mediaCarrierModeControl');
    const qualityControl = $('mediaVideoQualityControl');
    const rosterControl = $('mediaVideoRosterControl');
    if (modeControl && modeControl.value !== mode) modeControl.value = mode;
    if (qualityControl) qualityControl.value = state.settings.mediaVideoQuality || 'low';
    if (rosterControl) rosterControl.checked = Boolean(state.settings.mediaVideoShowRoster);
    if (start) {
      start.disabled = !supported || !state.presence || active || (mode === 'video' && !videoSupport);
      start.textContent = active ? 'Media Stream Running' : (mode === 'video' ? 'Start Status Video Stream' : 'Start Silent Audio Stream');
    }
    ...
    if (refreshStatuses) {
      refreshStatuses.disabled = !state.presence || state.pollInFlight || Boolean(state.uiLock);
      refreshStatuses.textContent = state.pollInFlight ? 'Refreshing Statuses…' : 'Refresh Statuses';
    }
    if (pip) pip.disabled = !state.presence || !('documentPictureInPicture' in window) || !window.isSecureContext;
    if (cast) {
      cast.disabled = !NativeCast || !castAppId || !nativeCastHttpsReady();
      cast.textContent = castStatus?.connected ? `Casting${castStatus.deviceName ? ` • ${castStatus.deviceName}` : ''}` : 'Cast Office Board';
    }
    if (castStop) {
      castStop.classList.toggle('hidden', !castStatus?.connected);
      castStop.disabled = !castStatus?.connected;
    }
    if (castLauncher) castLauncher.classList.toggle('hidden', !(castStatus?.ready && castAppId && nativeCastHttpsReady()));
    if (fullscreen) fullscreen.disabled = !(mode === 'video' && active);
    const previewWrap = $('statusVideoPreviewWrap');
    if (previewWrap) previewWrap.classList.toggle('hidden', mode !== 'video');
    renderStatusVideoFrame('media-ui-sync');
  }
  ...
  function updateMediaSession() {
    ...
    renderStatusVideoFrame('media-session-sync');
    syncNativeCastState('media-session-sync').catch(() => {});
    const metadataFingerprint = JSON.stringify([officeId, office, status, myStatus, peopleCount, mediaCarrierMode()]);
    if ('MediaMetadata' in window && metadataFingerprint !== state.mediaMetadataFingerprint) {
      try {
        // Clearing first forces mobile system media surfaces that cache metadata to discard
        // the previous office card before the replacement metadata is installed.
        navigator.mediaSession.metadata = null;
        navigator.mediaSession.metadata = new MediaMetadata({
          title: `${office} • ${status}`,
          artist: `Office ${status} • My status ${myStatus}`,
          album: `Statuses refresh continuously • Play keeps media active • Previous lowers • Next raises • ${peopleCount} ${peopleCount === 1 ? 'person' : 'people'} checked in`,
          artwork: [
            { src: './icon-192.png', sizes: '192x192', type: 'image/png' },
            { src: './icon-512.png', sizes: '512x512', type: 'image/png' }
          ]
        });
        state.mediaMetadataFingerprint = metadataFingerprint;
        Debug.info('media-session', 'System media metadata synchronised with current office', {
          officeId, office, officeStatus: status, myStatus, peopleCount
        });
      } catch (error) {
    ...
    setHandler('play', () => {
      startPreferredMediaStream({ source: 'media-session-play', resumeMonitoring: true })
        .catch(error => Debug.warn('media-session', 'Play media action could not start preferred carrier', { error }));
    });
    setHandler('pause', () => pauseMediaStream({ pauseMonitoring: false, notify: true }));
    setHandler('stop', () => {
      stopMediaStream({ clearSession: true, remember: false, notify: false }).catch(error => Debug.warn('media-session', 'Stop media action failed', { error }));
    });
    ...
    try { navigator.mediaSession.playbackState = mediaCarrierIsPlaying() ? 'playing' : 'paused'; } catch {}
    ...
    Debug.debug('media-session', 'Media Session metadata and monitoring controls updated', {
      ...
      mediaCarrierMode: mediaCarrierMode(),
      mediaStreamActive: mediaCarrierIsPlaying(),
      statusVideoRemoteState: state.mediaVideoRemoteState
    });
  }
  ...
  function currentPipSnapshot() {
    ...
    if (!ownOfficeId) return null;

    // Never reuse a snapshot from the office the user was previously checked into.
    // This is especially important for Media Session because mobile OS media cards can stay
    // visible while a new office snapshot is still being fetched.
    if (state.mode === 'admin' && state.adminSnapshot) {
    ...
    if (state.snapshot?.officeId === ownOfficeId) {
      return {
        ...state.snapshot,
        officeName: officeById(ownOfficeId)?.name || state.snapshot.officeName || ownOfficeId
      };
    }

    const directoryOffice = mediaOfficeDirectory().find(item => item.officeId === ownOfficeId);
    if (directoryOffice) {
      return {
        officeId: ownOfficeId,
        officeName: directoryOffice.officeName || officeById(ownOfficeId)?.name || ownOfficeId,
        people: directoryOffice.people || [],
        messages: [],
        officeStatus: directoryOffice.officeStatus || deriveOfficeStatus(directoryOffice.people || []),
        awaitingOfficeSnapshot: true
      };
    }

    // During an office switch use the new presence immediately rather than showing the old office.
    return {
      officeId: ownOfficeId,
      officeName: officeById(ownOfficeId)?.name || ownOfficeId,
      people: [state.presence],
      messages: [],
      officeStatus: String(state.presence.status || 'green').toLowerCase(),
      awaitingOfficeSnapshot: true
    };
  }
  ...
  function renderAdminFiltered() {
    ...
    const filteredPeople = allPeople.filter(p => {
      ...
      if (search && !`${p.name} ${p.email} ${p.displayEmail || ''} ${p.authType || ''} ${p.note || ''}`.toLowerCase().includes(search)) return false;
      return true;
    });
    ...
    $('adminOffices').innerHTML = ids.length ? ids.map(id => {
      ...
      return `<article class="card admin-office-card">
        ...
        <div class="admin-person-grid">${officePeople.map(personRowHtml).join('') || `<div class="empty-state">${escapeHtml(emptyText)}</div>`}</div>
      </article>`;
  ...
  function renderAdminMessages(messages) {
    ...
    const filtered = messages.filter(message => {
      if (officeFilter) {
        if (isStatusEscalationMessage(message)) {
          if (message.sourceOfficeId !== officeFilter) return false;
        } else if (message.officeId !== officeFilter && message.officeId !== '*') return false;
      }
      if (search && !`${message.authorName} ${message.authorEmail} ${message.authorDisplayEmail || ''} ${message.subjectName || ''} ${message.subjectEmail || ''} ${message.subjectDisplayEmail || ''} ${message.sourceOfficeName || ''} ${message.text}`.toLowerCase().includes(search)) return false;
      return true;
    });
    $('adminMessageHistory').classList.toggle('empty-state', !filtered.length);
    $('adminMessageHistory').innerHTML = filtered.length ? filtered.map(messageRowHtml).join('') : 'No recent messages match the current filters.';
  }

  function renderActivity(activity) {
    $('activityFeed').classList.toggle('empty-state', !activity.length);
    $('activityFeed').innerHTML = activity.length ? activity.map(item => `<div class="activity-row"><div class="activity-meta">${formatTime(item.createdAt)} • ${escapeHtml(item.actorName || displayEmailFor({email:item.actorEmail,displayEmail:item.actorDisplayEmail,authType:item.authType}) || 'System')}${item.authType === 'guest' ? ' • Guest' : ''} • ${escapeHtml(item.officeName || item.officeId || 'System')}</div><p><strong>${escapeHtml(item.action)}</strong>${item.summary ? ` • ${escapeHtml(item.summary)}` : ''}</p></div>`).join('') : 'No recent activity.';
  }
  ...
  async function performPoll({ manual = false } = {}) {
    ...
    try {
      ...
      state.lastPollSuccessAt = Date.now();
  ...
  function startWorker() {
    ...
    // Office status polling stays active for the whole checked-in session. Media controls no
    // longer own or pause this safety refresh loop.
    state.liveMonitoringPaused = false;
    if (!state.worker) {
  ...
  function startHeartbeat() {
    ...
    state.heartbeat = setInterval(async () => {
      ...
      try {
        ...
        if (data?.presence) syncPresenceState(data.presence, { source: 'heartbeat', updateSelectedStatus: true });
  ...
  function openSettings() {
    ...
    $('mediaCarrierModeSetting').value = mediaCarrierMode();
    $('mediaVideoQualitySetting').value = state.settings.mediaVideoQuality || 'low';
    $('mediaVideoRosterSetting').checked = Boolean(state.settings.mediaVideoShowRoster);
    $('castReceiverAppIdSetting').value = configuredCastReceiverAppId();
  ...
  async function savePreferences(event) {
    ...
    try {
      await runExclusive('Saving Settings', async () => {
        ...
        const previousMediaMode = mediaCarrierMode();
        state.settings.mediaCarrierMode = $('mediaCarrierModeSetting').value === 'video' ? 'video' : 'audio';
        state.settings.mediaVideoQuality = $('mediaVideoQualitySetting').value === 'standard' ? 'standard' : 'low';
        state.settings.mediaVideoShowRoster = $('mediaVideoRosterSetting').checked;
        const previousCastAppId = configuredCastReceiverAppId();
        state.settings.castReceiverAppId = String($('castReceiverAppIdSetting').value || '').trim();
        if (!state.settings.mediaControls && mediaCarrierIsPlaying()) await stopMediaStream({ clearSession: true, remember: false });
        else if (mediaCarrierIsPlaying() && previousMediaMode !== mediaCarrierMode()) await stopMediaStream({ clearSession: true, remember: true });
        ...
        saveSettings();
        if (previousCastAppId !== configuredCastReceiverAppId()) {
          stopNativeCastSession({ notify: false });
          await initialiseNativeCast({ silent: true });
        }
        startWorker();
        updateNotificationSettingsUi();
        await updateQueuedCount();
  ...
  async function signOut({ skipServer = false } = {}) {
    const perform = async () => {
      if (!skipServer && state.presence && hasAuthSession()) {
        try { await api('checkOut', { clientRequestId: makeClientRequestId('signout-checkout') }); } catch {}
      }
      if (!skipServer && state.guestToken) {
        try { await api('guestSignOut'); } catch {}
      }
      stopRealtime();
      closePip();
      await stopMediaStream({ clearSession: true, remember: true });
      ...
      clearSessionToken();
      clearGuestSessionToken();
      state.token = ''; state.guestToken = ''; state.user = null; state.presence = null; state.snapshot = null; state.adminSnapshot = null; state.mediaMonitorOfficeId = '';
  ...
  function bindEvents() {
    $('guestModeBtn')?.addEventListener('click', () => setGuestFormOpen(true));
    $('guestCancelBtn')?.addEventListener('click', () => setGuestFormOpen(false));
    ...
    $('refreshStatusesBtn')?.addEventListener('click', manualRefresh);
    ...
    $('startMediaStreamBtn').addEventListener('click', () => {
      startPreferredMediaStream({ source: 'media-controls-dialog', resumeMonitoring: true })
        .then(() => toast(mediaCarrierMode() === 'video'
          ? 'Office status video started for local preview, PiP or manual tab casting. Use Cast Office Board for native Google Cast.'
          : 'Silent audio stream started. System media controls should now be available where your browser exposes them.'))
        .catch(error => toast(error.message || 'Could not start media controls.', 'error'));
    });
    $('stopMediaStreamBtn').addEventListener('click', () => stopMediaStream({ clearSession: true, remember: false, notify: true }).catch(error => toast(error.message || 'Could not stop media stream.', 'error')));
    $('mediaCarrierModeControl').addEventListener('change', event => {
      state.settings.mediaCarrierMode = event.target.value === 'video' ? 'video' : 'audio';
      $('mediaCarrierModeSetting').value = state.settings.mediaCarrierMode;
      saveSettings();
      if (mediaCarrierIsPlaying()) {
        stopMediaStream({ clearSession: false, remember: true }).then(() => {
          toast('Media mode changed. Tap Start Media Stream to start the new carrier.', 'warn');
          updateMediaControlUi();
        });
      } else updateMediaControlUi();
    });
    $('mediaVideoQualityControl').addEventListener('change', event => {
      state.settings.mediaVideoQuality = event.target.value === 'standard' ? 'standard' : 'low';
      $('mediaVideoQualitySetting').value = state.settings.mediaVideoQuality;
      saveSettings();
      renderStatusVideoFrame('quality-change');
      if (mediaCarrierMode() === 'video' && mediaCarrierIsPlaying()) toast('Video quality changed. Restart the status video stream to apply the capture size.', 'warn');
    });
    $('mediaVideoRosterControl').addEventListener('change', event => {
      state.settings.mediaVideoShowRoster = event.target.checked;
      $('mediaVideoRosterSetting').checked = state.settings.mediaVideoShowRoster;
      saveSettings();
      renderStatusVideoFrame('roster-visibility-change');
    });
    $('mediaCastBtn').addEventListener('click', () => promptStatusVideoCast().catch(error => toast(error.message || 'Could not start native Cast.', 'error')));
    $('mediaCastStopBtn')?.addEventListener('click', () => stopNativeCastSession({ notify: true }));
    $('mediaFullscreenBtn').addEventListener('click', () => openStatusVideoFullscreen().catch(error => toast(error.message || 'Could not open TV view.', 'error')));
    ...
    $('mediaRefreshStatusesBtn').addEventListener('click', manualRefresh);
    ...
    document.addEventListener('visibilitychange', () => {
      if (document.visibilityState === 'visible') {
        ...
        if (state.presence) { state.liveMonitoringPaused = false; startWorker(); if (!state.heartbeat) startHeartbeat(); }
        pollNow();
      }
    });
        ...
        (async () => {
          try {
            ...
            if (task === 'periodic-refresh') {
              ...
              if (hasAuthSession() && !state.uiLock && !state.pollInFlight) await performPoll();
              port?.postMessage({ ok:true, task, outbox, refreshed: hasAuthSession() });
              return;
            }
  ...
  initialiseNativeCast({ silent: true });
  initBackendBridge();
  initGoogle();
