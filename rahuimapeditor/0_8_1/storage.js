import { createBlankRahui, normalizeRahui, normalizeSubmission, publicRecord, submissionToDraftRahui } from './model.js';

const LOCAL_DATA_KEY = 'rahui-map-data-v0.1';
const LOCAL_PUBLIC_CONFIG_KEY = 'rahui-map-public-config-v0.1';
const LOCAL_SUBMISSIONS_KEY = 'rahui-map-submissions-v0.1';
...
export class LocalStorageAdapter {
  ...
  async listSubmissions() {
    if (!this.editor) throw new Error('Submission review is only available in the editor.');
  ...
  async updateSubmission(id, changes = {}) {
    ...
    const next = normalizeSubmission({ ...items[index], ...changes, id: items[index].id, updatedAt: new Date().toISOString() });
    items[index] = next;
    writeLocalSubmissions(items);
    return next;
  }

  async convertSubmission(id, reviewNotes = '') {
    if (!this.editor) throw new Error('Submission review is only available in the editor.');
    const submissions = readLocalSubmissions();
    const index = submissions.findIndex(item => item.id === id);
    if (index < 0) throw new Error('Submission not found.');
    const submission = normalizeSubmission(submissions[index]);
    if (submission.status === 'converted' && submission.rahuiId) {
      const existing = readLocalItems().find(item => item.id === submission.rahuiId);
      if (existing) return { submission, rahui: existing };
    }
    const draft = submissionToDraftRahui(submission);
    const saved = await this.save(draft);
    const now = new Date().toISOString();
    const updated = normalizeSubmission({ ...submission, status: 'converted', rahuiId: saved.id, reviewNotes, reviewedAt: now, updatedAt: now });
    submissions[index] = updated;
    writeLocalSubmissions(submissions);
    return { submission: updated, rahui: saved };
  }

  async health() {
    return { ok: true, provider: 'local', count: readLocalItems().length, submissions: readLocalSubmissions().length };
  }
...
export class GasStorage {
  ...
  async submitSubmission(submission) {
    return this.post({ action: 'submitSubmission', submission });
  }

  async listSubmissions() {
    if (!this.editor) throw new Error('Submission review is only available in the editor.');
    return this.post({ action: 'listSubmissions', token: this.token, editorName: this.editorName });
  }

  async updateSubmission(id, changes = {}) {
    if (!this.editor) throw new Error('Submission review is only available in the editor.');
    return this.post({ action: 'updateSubmission', token: this.token, editorName: this.editorName, id, changes });
  }

  async convertSubmission(id, reviewNotes = '') {
    if (!this.editor) throw new Error('Submission review is only available in the editor.');
    return this.post({ action: 'convertSubmission', token: this.token, editorName: this.editorName, id, reviewNotes });
  }
...
export function readLocalSubmissions() {
  try {
    const parsed = JSON.parse(localStorage.getItem(LOCAL_SUBMISSIONS_KEY) || '[]');
...
export function writeLocalSubmissions(items) {
  localStorage.setItem(LOCAL_SUBMISSIONS_KEY, JSON.stringify(items.map(normalizeSubmission)));
}
