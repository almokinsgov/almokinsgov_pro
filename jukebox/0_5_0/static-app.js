...
function peerIdForRoom(code) {
  return `${config.peerPrefix || 'gnj-v050-'}${String(code || '').toLowerCase()}`;
}
...
function publicCollection(collection, { includeTracks = false } = {}) {
  return {
    ...
    owner: collection.owner || '', image: collection.image || '', uri: collection.uri || '',
...
async function listSpotifyCollections(type) {
  ...
  if (type === 'playlists') {
    ...
    return (data.items || []).filter(Boolean).map((item) => ({ id: item.id, type: 'playlist', name: item.name, description: item.description || '', owner: item.owner?.display_name || '', image: item.images?.[0]?.url || '', uri: item.uri || '', trackCount: item.tracks?.total || 0 }));
  }
  const data = await spotifyFetch('/me/albums?limit=20');
  return (data.items || []).map((entry) => entry.album).filter(Boolean).map((item) => ({ id: item.id, type: 'album', name: item.name, description: (item.artists || []).map((artist) => artist.name).join(', '), owner: (item.artists || []).map((artist) => artist.name).join(', '), image: item.images?.[0]?.url || '', uri: item.uri || '', trackCount: item.total_tracks || 0 }));
}
...
async function getSpotifyCollection(type, id) {
  ...
  if (type === 'playlist') {
    const data = await spotifyFetch(`/playlists/${encodeURIComponent(id)}?fields=id,name,description,images,uri,owner(display_name),tracks(total,items(track))`);
    const tracks = (data.tracks?.items || []).map((entry) => entry.track).filter(Boolean).map(normaliseSpotifyTrack);
    tracks.forEach(cacheTrack);
    return { id: data.id, type, name: data.name, description: data.description || '', owner: data.owner?.display_name || '', image: data.images?.[0]?.url || '', uri: data.uri || '', trackCount: data.tracks?.total || tracks.length, tracks };
  }
  ...
  tracks.forEach(cacheTrack);
  return { id: data.id, type, name: data.name, description: owner, owner, image: data.images?.[0]?.url || '', uri: data.uri || '', trackCount: data.total_tracks || tracks.length, tracks };
}
...
function updatePipView() {
  ...
  if (!root) return;
  const playing = state.room?.nowPlaying;
  const visible = playing && !playbackEffectivelyEnded(playing);
