...
function peerIdForRoom(code) {
  return `${config.peerPrefix || 'gnj-v030-'}${String(code || '').toLowerCase()}`;
}
...
async function startQueueItem(item, positionMs = 0, { archive = true } = {}) {
  const room = state.roomInternal;
  clearPlaybackTimer();
  if (!MOCK_MODE) {
  ...
  if (archive && room.nowPlaying) archiveNowPlaying('skipped');
  const now = Date.now();
  room.nowPlaying = {
    track: item.track,
...
async function playNext(forcedItemId = '', fromAuto = false) {
  ...
  await startQueueItem(item, 0, { archive: Boolean(room.nowPlaying) });
  return true;
}
...
function renderNowPlaying() {
  ...
  if (!playing) {
    ...
    refs.pauseResumeButton.textContent = 'Pause';
  ...
  const { track } = playing;
  refs.nowPlayingTitle.textContent = track.name;
  const pieces = [track.artists];
  if (playing.requestedBy && playing.requestedBy !== 'Spotify') pieces.push(`requested by ${playing.requestedBy}`);
  if (state.room.selectedDeviceName) pieces.push(state.room.selectedDeviceName);
  if (!playing.isPlaying) pieces.push('paused');
  refs.nowPlayingMeta.textContent = pieces.join(' · ');
  refs.guestPlaybackControls.classList.toggle('hidden', state.isHost || playing.isPlaying);
  refs.guestStartButton.textContent = 'Resume playback';
  refs.guestPlaybackHint.textContent = 'Playback is paused. Resume the current track without skipping it.';
  ...
  refs.pauseResumeButton.textContent = playing.isPlaying ? 'Pause' : 'Resume';
  updateProgress();
}
...
function renderRoom() {
  ...
  refs.roomView.classList.remove('hidden');
...
refs.pauseResumeButton.addEventListener('click', () => hostPlayerAction(state.room?.nowPlaying?.isPlaying ? 'pause' : 'resume'));
