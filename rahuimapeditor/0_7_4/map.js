...
export function relativeDateText(kind, value, referenceDate = new Date()) {
  ...
  const amount = `${Math.abs(difference)} ${Math.abs(difference) === 1 ? 'day' : 'days'}`;

  const labels = {
    notified: { future: `Notification in ${amount}`, today: 'Notification today', past: `Notified ${amount} ago` },
    starts: { future: `Starts in ${amount}`, today: 'Starts today', past: `Started ${amount} ago` },
    ends: { future: `Ends in ${amount}`, today: 'Ends today', past: `Ended ${amount} ago` },
    review: { future: `Review in ${amount}`, today: 'Review today', past: `Review date was ${amount} ago` },
    lifted: { future: `To be lifted in ${amount}`, today: 'To be lifted today', past: `Was lifted ${amount} ago` }
  };
  const phrases = labels[kind] || { future: `In ${amount}`, today: 'Today', past: `${amount} ago` };
  return difference > 0 ? phrases.future : difference < 0 ? phrases.past : phrases.today;
}
