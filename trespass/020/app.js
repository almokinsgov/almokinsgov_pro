(() => {
  ...
  function loadSettings() {
    try {
      ...
      return {
        ...
        quickControls: false, mediaControls: false
      };
    }
  }
  ...
  function updateMediaSession() {
    ...
    setHandler('play', () => {
      ...
      pollNow();
      updateMediaSession();
      toast('Live roster refresh resumed.');
    });
    setHandler('pause', () => {
      state.liveMonitoringPaused = true;
      state.worker?.postMessage({ type:'stop' });
      updateMediaSession();
      toast('Live roster refresh paused. Your presence heartbeat is still active.', 'warn');
    });
    setHandler('stop', () => {
      state.liveMonitoringPaused = true;
      state.worker?.postMessage({ type:'stop' });
    ...
    try { navigator.mediaSession.playbackState = state.liveMonitoringPaused ? 'paused' : 'playing'; } catch {}
    state.mediaSessionActive = true;
    Debug.debug('media-session', 'Media Session metadata and safe monitoring controls updated', {
      office,
      status,
      peopleCount,
      myStatus,
  ...
  async function signOut({ skipServer = false } = {}) {
    const perform = async () => {
      ...
      $('signOutBtn').classList.add('hidden'); $('settingsBtn').classList.add('hidden'); $('pipBtn').classList.add('hidden'); $('quickControlsBtn')?.classList.add('hidden');
