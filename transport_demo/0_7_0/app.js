(() => {
  ...
  const suppliedConfig = window.TRANSPORT_PAGE_BROWSER_CONFIG || {};
  const features = Object.freeze({
    ...DEFAULT_FEATURES,
    ...(suppliedConfig.features || {})
  });
  ...
  const el = {
    ...
    showRemovedControl: document.getElementById('showRemovedControl'),
    ...
    saveInfoCard: document.getElementById('saveInfoCard'),
    ...
    includeImagesControl: document.getElementById('includeImagesControl'),
    includePlaceholdersControl: document.getElementById('includePlaceholdersControl')
  };
  ...
  function setFeatureVisibility(element, visible) {
    if (!element) return;
    element.hidden = !visible;
    element.classList.toggle('feature-hidden', !visible);
    element.setAttribute('aria-hidden', String(!visible));
  }

  function applyFeatureConfig() {
    setFeatureVisibility(el.showRemovedControl, features.showRemovedControl);
    setFeatureVisibility(el.tabEdit, features.editorMode);
    setFeatureVisibility(el.tabCode, features.codeView);
    setFeatureVisibility(el.includeImagesControl, features.images);
    setFeatureVisibility(el.includePlaceholdersControl, features.dynamicPlaceholders);
    setFeatureVisibility(el.saveInfoCard, features.saveInfo);
    setFeatureVisibility(el.historyCard, features.history && features.editorMode);

    [el.saveWorkspace, el.savePageTop, el.savePage].forEach(node => setFeatureVisibility(node, features.saveControls));
    [el.resetWorkspace, el.resetVersion, el.resetPage].forEach(node => setFeatureVisibility(node, features.restoreControls));
    [el.undoPage, el.redoPage, el.historyUndo, el.historyRedo].forEach(node => setFeatureVisibility(node, features.history && features.restoreControls));
    [el.importWorkspace, el.exportWorkspace].forEach(node => setFeatureVisibility(node, features.workspaceImportExport));
    document.querySelectorAll('.metadata-copy-control').forEach(node => setFeatureVisibility(node, features.metadataCopy));
    document.querySelectorAll('.copy-options, .copy-actions').forEach(node => setFeatureVisibility(node, features.copyTools));
    setFeatureVisibility(el.accordionMode?.closest('.field-group'), features.copyTools);
    document.querySelectorAll('.history-info-item').forEach(node => setFeatureVisibility(node, features.history));

    const editorSaveHelp = document.querySelector('.editor-save-state span');
    if (editorSaveHelp && !features.saveControls) {
      editorSaveHelp.textContent = 'Autosave protects drafts. Manual page and workspace save checkpoints are disabled by config.';
    }

    if (!features.images) el.includeImages.checked = false;
    if (!features.dynamicPlaceholders) el.includePlaceholders.checked = false;
    if (!features.showRemovedControl) {
      state.showRemoved = false;
      el.showRemoved.checked = false;
    }
    if (!features.editorMode && state.view === 'edit') state.view = 'page';
    if (!features.codeView && state.view === 'code') state.view = 'page';

    document.body.classList.toggle('feature-images-disabled', !features.images);
    document.body.classList.toggle('feature-placeholders-disabled', !features.dynamicPlaceholders);
    document.body.classList.toggle('feature-editor-disabled', !features.editorMode);
    document.body.classList.toggle('feature-history-disabled', !features.history);
  }
  ...
  function undoPageHistory() {
    if (!features.history || !features.restoreControls) return;
  ...
  function renderHistory() {
    if (!features.history || !features.editorMode) return;
    ...
    history.entries.map((entry, index) => ({ entry, index })).reverse().forEach(({ entry, index }) => {
      ...
      } else if (features.restoreControls) {
  ...
  function selectPage(id, { updateHash = true } = {}) {
    ...
    if (!page) return;
    if (page.removed && !state.showRemoved && !features.showRemovedControl) return;
    if (page.removed && !state.showRemoved && features.showRemovedControl) {
  ...
  function cleanCmsHtml() {
    ...
    if (!features.images || !el.includeImages.checked) {
      wrapper.querySelectorAll('[data-demo-media], figure.content-image, img').forEach(node => node.remove());
    }
    if (!features.dynamicPlaceholders || !el.includePlaceholders.checked) {
      wrapper.querySelectorAll('[data-cms-placeholder="dynamic"], [data-content-placeholder="true"], .cms-placeholder, .content-placeholder').forEach(node => node.remove());
    }
  ...
  function renderStatusAndMetadata(page) {
    ...
    const mediaParts = [];
    if (features.images && media.images) mediaParts.push(`${media.images} image${media.images === 1 ? '' : 's'}`);
    if (features.dynamicPlaceholders && media.placeholders) mediaParts.push(`${media.placeholders} dynamic placeholder${media.placeholders === 1 ? '' : 's'}`);
  ...
  function switchView(view) {
    if (state.view === 'edit' && view !== 'edit' && features.editorMode) commitEditorToPage({ silent: true });
    const allowedViews = ['page'];
    if (features.codeView) allowedViews.push('code');
    if (features.editorMode) allowedViews.push('edit');
    state.view = allowedViews.includes(view) ? view : 'page';
  ...
  function applyUiState(saved = {}) {
    state.showRemoved = features.showRemovedControl ? Boolean(saved.showRemoved) : false;
    ...
    el.includeImages.checked = features.images && saved.includeImages !== false;
    el.includePlaceholders.checked = features.dynamicPlaceholders && saved.includePlaceholders !== false;
  ...
  function saveCurrentPageCheckpoint() {
    if (!features.saveControls) return;
    if (state.view === 'edit') commitEditorToPage({ silent: true });
    const page = currentPage();
    if (!page) return;
  ...
  function resetCurrentVersion() {
    if (!features.restoreControls) return;
    if (state.view === 'edit' && editorContext.dirty) commitEditorToPage({ silent: true });
    const page = currentPage();
    if (!page) return;
    const base = baseById.get(page.id);
  ...
  function exportWorkspace() {
    if (!features.workspaceImportExport) return;
    if (state.view === 'edit') commitEditorToPage({ silent: true });
  ...
  async function importWorkspaceFile(file) {
    ...
    try {
      ...
      const importedSelected = byId.get(state.selectedId);
      if (!importedSelected || (!state.showRemoved && importedSelected.removed)) {
        state.selectedId = visiblePages()[0]?.id || pages[0]?.id || null;
      }
      persistWorkspace({ silent: true });
      renderTree();
  ...
  document.addEventListener('keydown', event => {
    ...
    if ((event.ctrlKey || event.metaKey) && event.shiftKey && event.key.toLowerCase() === 's') {
      event.preventDefault();
      if (features.saveControls) saveWorkspaceCheckpoint();
    } else if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 's') {
      event.preventDefault();
      if (!features.saveControls) return;
      if (state.view === 'edit' && features.editorMode) saveCurrentPageCheckpoint();
      else saveWorkspaceCheckpoint();
    }
    if ((event.ctrlKey || event.metaKey) && event.shiftKey && event.key.toLowerCase() === 'c') {
      event.preventDefault();
      if (features.copyTools) el.copyWysiwyg.click();
    }
  });
  ...
  const hashPage = byId.get(hashId);
  const savedPage = byId.get(state.selectedId);
  const hashAllowed = hashPage && (state.showRemoved || !hashPage.removed);
  const savedAllowed = savedPage && (state.showRemoved || !savedPage.removed);
  const start = hashAllowed ? hashId : (savedAllowed ? state.selectedId : null) || visiblePages()[0]?.id || pages[0]?.id;
