(() => {
  ...
  let driveReconcileContext = null;
  ...
  function cacheElements() {
    const ids = [
      ...
      'googleDriveDialog', 'closeGoogleDriveButton', 'doneGoogleDriveButton', 'googleSignedOutPanel', 'googleSignedInPanel', 'googleOriginNote', 'googleSignInButton', 'googleSyncNowButton', 'googleSignOutButton', 'googleProfileImage', 'googleProfileName', 'googleProfileEmail', 'googleSyncStatus', 'googleClientIdDisplay', 'googleReconcilePanel', 'googleReconcileMessage', 'googleUseDriveButton', 'googleMergeWorkspacesButton', 'googleReplaceDriveButton', 'googleRecoveryPanel', 'googleRecoveryMessage', 'googleRestoreRecoveryButton',
  ...
  function bindEvents() {
    ...
    el.googleUseDriveButton.addEventListener('click', resolveDriveUseRemote);
    el.googleMergeWorkspacesButton.addEventListener('click', resolveDriveMerge);
    el.googleReplaceDriveButton.addEventListener('click', resolveDriveUseLocal);
    el.googleRestoreRecoveryButton.addEventListener('click', restoreDriveRecovery);
  ...
  function loadState() {
    try {
      ...
      if (!parsed) return normaliseWorkspace(DEFAULT_STATE);
  ...
  function normaliseWorkspace(input) {
    ...
    return {
      ...base,
      ...source,
      schemaVersion: Math.max(3, Number(source.schemaVersion) || 1),
      workspaceId: source.workspaceId || uid(),
      cloud: { ...base.cloud, ...(source.cloud || {}) },
  ...
  function normaliseScript(script, preferences = state?.preferences || DEFAULT_STATE.preferences) {
    ...
    return {
      ...
      starterTemplate: Boolean(script.starterTemplate),
  ...
  function ensureInitialScript() {
    if (!state.scripts.length) {
      const script = makeScript('Welcome to Teleprompter', 'Write or paste your script here.\n\nOpen the player when you are ready. Use Space to play or pause and the arrow keys to adjust speed.', null, { starterTemplate: true });
  ...
  function makeScript(title = 'Untitled script', body = '', playerSettings = null, options = {}) {
    const now = new Date().toISOString();
    return {
      id: uid(),
      title,
      ...
      starterTemplate: Boolean(options.starterTemplate),
  ...
  async function handleImportFiles(event) {
    ...
    const profileBeforeImport = clone(state.profile);
      ...
      try {
        ...
        if (name.endsWith('.json')) {
          ...
          if (data && Array.isArray(data.scripts) && data.preferences) {
            ...
            state.workspaceId = replacement.workspaceId;
            state.cloud = replacement.cloud;
    ...
    ensureInitialScript();
    markWorkspaceChanged();
    if (workspaceImported) {
      const safety = getSyncSafety();
      const sync = getDriveSync();
      if (safety) {
        safety.clearAccountRecord(profileBeforeImport);
        safety.clearAccountRecord(state.profile);
        if (sync?.profile) safety.clearAccountRecord(sync.profile);
      }
      driveConflictPending = Boolean(sync?.isSignedIn());
      driveReconcileContext = null;
    ...
    if (workspaceImported && driveIsSignedIn()) await syncGoogleDriveNow({ manual: true, importedWorkspace: true });
  ...
  function getSyncSafety() {
    return window.TeleprompterSyncSafety || null;
  }
  ...
  function updateGoogleDriveUi() {
    ...
    if (el.googleReconcilePanel) {
      el.googleReconcilePanel.hidden = !signedIn || !driveReconcileContext;
      if (driveReconcileContext) {
        el.googleReconcileMessage.textContent = driveReconcileContext.message || 'This browser and Google Drive contain different workspace data.';
        const hasRemote = Boolean(driveReconcileContext.remoteWorkspace && driveReconcileContext.file);
        el.googleUseDriveButton.hidden = !hasRemote;
        el.googleMergeWorkspacesButton.hidden = !hasRemote;
        el.googleReplaceDriveButton.textContent = hasRemote ? 'Replace Drive With This Browser' : 'Create Drive Workspace From This Browser';
      }
    }

    const recovery = signedIn && safety ? safety.getLatestRecovery(sync.profile) : null;
    if (el.googleRecoveryPanel) {
      el.googleRecoveryPanel.hidden = !recovery;
      if (recovery) {
        const when = recovery.savedAt ? new Date(recovery.savedAt).toLocaleString('en-NZ') : 'recently';
        const source = recovery.source === 'remote' ? 'Drive copy' : 'local browser copy';
        el.googleRecoveryMessage.textContent = `${source} saved ${when}. Restore it locally without automatically overwriting Drive.`;
      }
    }
  }
  ...
  async function signInWithGoogle() {
    ...
    try {
      ...
      await sync.signIn();
      updateGoogleDriveUi();
      const result = await syncGoogleDriveNow({ manual: true, signingIn: true });
      if (!driveConflictPending && result) {
        const changed = applyGoogleProfileToWorkspace(sync.profile);
        if (changed) {
          markWorkspaceChanged();
          persistNow();
        }
      }
      toast(driveConflictPending ? 'Connected · review sync options' : `Connected ${sync.profile?.email || 'Google Drive'}`);
    } catch (error) {
  ...
  function applyGoogleProfileToObject(workspace, profile) {
    if (!workspace || !profile) return false;
    const before = JSON.stringify(workspace.profile || {});
    workspace.profile = { ...(workspace.profile || {}) };
    workspace.profile.provider = 'google';
    workspace.profile.googleSub = profile.sub || '';
    workspace.profile.email = profile.email || '';
    workspace.profile.picture = profile.picture || '';
    if (!workspace.profile.displayName && profile.name) workspace.profile.displayName = profile.name;
    return JSON.stringify(workspace.profile) !== before;
  }

  function applyGoogleProfileToWorkspace(profile) {
    const changed = applyGoogleProfileToObject(state, profile);
    if (changed) applyStateToUI();
    return changed;
  }
  ...
  function queueDriveReconciliation(context) {
    driveReconcileContext = {
      reason: context.reason || 'review',
      message: context.message || 'This browser and Google Drive contain different workspace data.',
      localWorkspace: clone(context.localWorkspace || state),
      remoteWorkspace: context.remoteWorkspace ? clone(context.remoteWorkspace) : null,
      file: context.file ? clone(context.file) : null
    };
    driveConflictPending = true;
    clearTimeout(driveSyncTimer);
    driveSyncTimer = null;
    handleGoogleDriveStatus({ state: 'conflict', message: 'Sync review needed · nothing overwritten' });
    updateGoogleDriveUi();
  }

  function clearDriveReconciliation() {
    driveConflictPending = false;
    driveReconcileContext = null;
    updateGoogleDriveUi();
  }

  function workspaceIsPristine(workspace) {
    const safety = getSyncSafety();
    return Boolean(safety && safety.isPristineStarterWorkspace(workspace, DEFAULT_STATE.preferences));
  }

  function prepareCloudWorkspace(sourceWorkspace, remoteWorkspace = null) {
    const safety = getSyncSafety();
    const prepared = normaliseWorkspace(clone(sourceWorkspace));
    const remote = remoteWorkspace ? normaliseWorkspace(clone(remoteWorkspace)) : null;
    if (remote?.workspaceId) prepared.workspaceId = remote.workspaceId;
    if (!prepared.workspaceId) prepared.workspaceId = uid();
    const currentRevision = Math.max(Number(prepared.cloud?.revision) || 0, Number(remote?.cloud?.revision) || 0);
    prepared.cloud = {
      revision: currentRevision + 1,
      updatedAt: new Date().toISOString(),
      updatedBy: safety?.getDeviceId?.() || ''
    };
    prepared.schemaVersion = 3;
    prepared.appVersion = APP_VERSION;
    return prepared;
  }

  async function commitWorkspaceToDrive(sourceWorkspace, file = null, remoteWorkspace = null) {
    const sync = getDriveSync();
    const safety = getSyncSafety();
    let targetFile = file;
    let expectedRemote = remoteWorkspace;

    if (targetFile?.id && expectedRemote && safety) {
      const latest = await sync.fetchWorkspace();
      if (!latest.file || !latest.workspace) {
        const error = new Error('The Drive workspace changed or disappeared while syncing. Sync again before writing.');
        error.code = 'DRIVE_CHANGED_DURING_SYNC';
        throw error;
      }
      const latestWorkspace = normaliseWorkspace(latest.workspace);
      const expectedWorkspace = normaliseWorkspace(expectedRemote);
      if (!safety.sameWorkspaceContent(latestWorkspace, expectedWorkspace)) {
        const error = new Error('The Drive workspace changed on another device while this sync was running. Nothing was overwritten. Sync again to reconcile the new version.');
        error.code = 'DRIVE_CHANGED_DURING_SYNC';
        throw error;
      }
      targetFile = latest.file;
      expectedRemote = latestWorkspace;
    }

    const prepared = prepareCloudWorkspace(sourceWorkspace, expectedRemote);
    const savedFile = await sync.saveWorkspace(prepared, targetFile?.id || '', { createOnly: !targetFile });
    return { workspace: prepared, file: savedFile };
  }

  function establishDriveBaseline(workspace, file) {
    const sync = getDriveSync();
    const safety = getSyncSafety();
    if (!sync?.profile || !safety) return null;
    return safety.establishBaseline(sync.profile, workspace, file);
  }

  async function syncGoogleDriveNow(options = {}) {
    const sync = getDriveSync();
    const safety = getSyncSafety();
    if (!sync || !sync.isSignedIn() || !safety || driveSyncInFlight) return null;
    if (driveConflictPending && driveReconcileContext) {
      if (options.manual) toast('Choose a sync review option first');
      return { direction: 'review', reason: driveReconcileContext.reason };
    }
    ...
    try {
      flushEditorToState();
      persistNow({ cloud: false });
      const localWorkspace = normaliseWorkspace(clone(state));
      const profile = sync.profile;
      const record = safety.getAccountRecord(profile);
      const accountSwitched = Boolean(state.profile?.googleSub && profile?.sub && state.profile.googleSub !== profile.sub);
      const bundle = await sync.fetchWorkspace();

      if (!bundle.workspace || !bundle.file) {
        if (record?.fileId || accountSwitched) {
          queueDriveReconciliation({
            reason: accountSwitched ? 'account-switch-empty' : 'remote-missing',
            message: accountSwitched
              ? 'This browser contains a workspace associated with another Google account. The signed-in account has no Drive workspace. Nothing will be copied until you explicitly choose to create one.'
              : 'The Drive workspace that this browser previously synced with is missing. It may have been removed elsewhere. It will not be recreated automatically.',
            localWorkspace,
            remoteWorkspace: null,
            file: null
          });
          return { direction: 'review', reason: accountSwitched ? 'account-switch-empty' : 'remote-missing' };
        }

        const initial = clone(localWorkspace);
        applyGoogleProfileToObject(initial, profile);
        const committed = await commitWorkspaceToDrive(initial, null, null);
        applyRemoteWorkspace(committed.workspace);
        establishDriveBaseline(state, committed.file);
        handleGoogleDriveStatus({ state: 'synced', message: 'Created protected Drive workspace' });
        return { direction: 'upload', reason: 'created', file: committed.file };
      }

      let remoteWorkspace = normaliseWorkspace(bundle.workspace);
      const remoteHadIdentity = Boolean(bundle.workspace.workspaceId);
      const remoteHadRevision = Number(bundle.workspace?.cloud?.revision) > 0;
      const localPristine = workspaceIsPristine(localWorkspace);

      if (safety.sameWorkspaceContent(localWorkspace, remoteWorkspace)) {
        remoteWorkspace.workspaceId = bundle.workspace.workspaceId || localWorkspace.workspaceId || remoteWorkspace.workspaceId;
        if (!remoteHadIdentity || !remoteHadRevision) {
          const committed = await commitWorkspaceToDrive(remoteWorkspace, bundle.file, remoteWorkspace);
          remoteWorkspace = committed.workspace;
          bundle.file = committed.file;
        }
        applyRemoteWorkspace(remoteWorkspace);
        establishDriveBaseline(state, bundle.file);
        handleGoogleDriveStatus({ state: 'synced', message: bundle.duplicates ? 'Drive synced · duplicate workspace files detected' : 'Google Drive is up to date' });
        return { direction: 'none', reason: 'same-content', file: bundle.file };
      }

      if (localPristine && (!record || record.workspaceId !== remoteWorkspace.workspaceId)) {
        if (!remoteHadIdentity || !remoteHadRevision) {
          const committed = await commitWorkspaceToDrive(remoteWorkspace, bundle.file, remoteWorkspace);
          remoteWorkspace = committed.workspace;
          bundle.file = committed.file;
        ...
        applyRemoteWorkspace(remoteWorkspace);
        establishDriveBaseline(state, bundle.file);
        handleGoogleDriveStatus({ state: 'synced', message: 'Loaded existing Drive workspace on this browser' });
        toast('Loaded your existing Drive workspace');
        return { direction: 'download', reason: 'fresh-browser-protected', workspace: remoteWorkspace, file: bundle.file };
      }

      const localId = localWorkspace.workspaceId || '';
      const remoteId = remoteWorkspace.workspaceId || '';
      const recordMatches = Boolean(record?.baseWorkspace && record.workspaceId && record.workspaceId === localId && record.workspaceId === remoteId);

      if (recordMatches) {
        const baseWorkspace = normaliseWorkspace(record.baseWorkspace);
        const localChanged = !safety.sameWorkspaceContent(localWorkspace, baseWorkspace);
        const remoteChanged = !safety.sameWorkspaceContent(remoteWorkspace, baseWorkspace);

        if (!localChanged && !remoteChanged) {
          establishDriveBaseline(remoteWorkspace, bundle.file);
          handleGoogleDriveStatus({ state: 'synced', message: bundle.duplicates ? 'Drive synced · duplicate workspace files detected' : 'Google Drive is up to date' });
          return { direction: 'none', reason: 'baseline-current', file: bundle.file };
        }

        if (localChanged && !remoteChanged) {
          const candidate = clone(localWorkspace);
          applyGoogleProfileToObject(candidate, profile);
          const committed = await commitWorkspaceToDrive(candidate, bundle.file, remoteWorkspace);
          applyRemoteWorkspace(committed.workspace);
          establishDriveBaseline(state, committed.file);
          handleGoogleDriveStatus({ state: 'synced', message: 'Saved this browser’s changes to Drive' });
          return { direction: 'upload', reason: 'local-only-change', file: committed.file };
        }

        if (!localChanged && remoteChanged) {
          applyRemoteWorkspace(remoteWorkspace);
          establishDriveBaseline(state, bundle.file);
          handleGoogleDriveStatus({ state: 'synced', message: 'Loaded changes made on another device' });
          return { direction: 'download', reason: 'remote-only-change', workspace: remoteWorkspace, file: bundle.file };
        }

        const merged = safety.threeWayMerge(baseWorkspace, localWorkspace, remoteWorkspace);
        applyGoogleProfileToObject(merged.workspace, profile);
        const committed = await commitWorkspaceToDrive(merged.workspace, bundle.file, remoteWorkspace);
        applyRemoteWorkspace(committed.workspace);
        establishDriveBaseline(state, committed.file);
        const conflictText = merged.conflicts
          ? `Merged device and Drive changes · ${merged.conflicts} conflict${merged.conflicts === 1 ? '' : 's'} preserved`
          : 'Merged changes from this browser and Drive';
        handleGoogleDriveStatus({ state: 'synced', message: conflictText });
        toast(conflictText);
        return { direction: 'merge', reason: 'both-changed', conflicts: merged.conflicts, file: committed.file };
      }

      const sameIdentity = Boolean(localId && remoteId && localId === remoteId);
      queueDriveReconciliation({
        reason: sameIdentity ? 'baseline-missing' : (accountSwitched ? 'account-switch' : 'different-workspaces'),
        message: accountSwitched
          ? 'This browser was previously associated with a different Google account and the signed-in account already has a Drive workspace. Review the copies before anything is changed.'
          : sameIdentity
            ? 'This workspace exists in both places but this browser no longer has a trusted sync baseline, for example after browser storage was partly cleared or a workspace backup was imported. Review the copies before syncing.'
            : 'This browser and Google Drive contain different workspaces. This can happen on first sign-in from a browser that already has local scripts. Nothing will be overwritten automatically.',
        localWorkspace,
        remoteWorkspace,
        file: bundle.file
      });
      ...
      if (error?.code === 'DRIVE_CHANGED_DURING_SYNC') {
        handleGoogleDriveStatus({ state: 'conflict', message: 'Drive changed during sync · retrying safely' });
        setTimeout(() => syncGoogleDriveNow({ manual: false }), 300);
      } else {
  ...
  async function resolveDriveUseRemote() {
    const sync = getDriveSync();
    const safety = getSyncSafety();
    const context = driveReconcileContext;
    if (!sync?.isSignedIn() || !safety || !context?.remoteWorkspace || !context.file || driveSyncInFlight) return;
    driveSyncInFlight = true;
    try {
      safety.saveRecoverySnapshot(context.localWorkspace, { reason: 'before-use-drive', source: 'local', profile: sync.profile });
      const remote = clone(context.remoteWorkspace);
      applyGoogleProfileToObject(remote, sync.profile);
      const committed = await commitWorkspaceToDrive(remote, context.file, context.remoteWorkspace);
      applyRemoteWorkspace(committed.workspace);
      establishDriveBaseline(state, committed.file);
      clearDriveReconciliation();
      handleGoogleDriveStatus({ state: 'synced', message: 'Using Google Drive workspace' });
      toast('Drive workspace loaded · previous local copy saved for recovery');
    } catch (error) {
      handleGoogleDriveStatus({ state: 'error', message: error.message || 'Could not load Drive workspace' });
      toast(error.message || 'Could not load Drive workspace');
    } finally {
      driveSyncInFlight = false;
      updateGoogleDriveUi();
    }
  ...
  async function resolveDriveMerge() {
    const sync = getDriveSync();
    const safety = getSyncSafety();
    const context = driveReconcileContext;
    if (!sync?.isSignedIn() || !safety || !context?.remoteWorkspace || !context.file || driveSyncInFlight) return;
    driveSyncInFlight = true;
    try {
      safety.saveRecoverySnapshot(context.localWorkspace, { reason: 'before-merge', source: 'local', profile: sync.profile });
      const merged = safety.mergeUnrelatedWorkspaces(context.remoteWorkspace, context.localWorkspace);
      merged.workspace.workspaceId = context.remoteWorkspace.workspaceId || merged.workspace.workspaceId;
      applyGoogleProfileToObject(merged.workspace, sync.profile);
      const committed = await commitWorkspaceToDrive(merged.workspace, context.file, context.remoteWorkspace);
      applyRemoteWorkspace(committed.workspace);
      establishDriveBaseline(state, committed.file);
      clearDriveReconciliation();
      const message = `Merged workspaces · ${merged.added} local script${merged.added === 1 ? '' : 's'} added${merged.conflicts ? ` · ${merged.conflicts} ID conflict${merged.conflicts === 1 ? '' : 's'} preserved` : ''}`;
      handleGoogleDriveStatus({ state: 'synced', message });
      toast(message);
    } catch (error) {
      handleGoogleDriveStatus({ state: 'error', message: error.message || 'Could not merge workspaces' });
      toast(error.message || 'Could not merge workspaces');
    } finally {
      driveSyncInFlight = false;
      updateGoogleDriveUi();
    }
  ...
  async function resolveDriveUseLocal() {
    const sync = getDriveSync();
    const safety = getSyncSafety();
    const context = driveReconcileContext;
    if (!sync?.isSignedIn() || !safety || !context || driveSyncInFlight) return;
    const hasRemote = Boolean(context.remoteWorkspace && context.file);
    if (hasRemote) {
      const confirmed = window.confirm('Replace the Google Drive workspace with this browser’s copy?\n\nThe current Drive copy will be saved as a local recovery snapshot first.');
      if (!confirmed) return;
    }
    driveSyncInFlight = true;
    try {
      if (hasRemote) safety.saveRecoverySnapshot(context.remoteWorkspace, { reason: 'before-replace-drive', source: 'remote', profile: sync.profile });
      const local = clone(context.localWorkspace || state);
      if (context.remoteWorkspace?.workspaceId) local.workspaceId = context.remoteWorkspace.workspaceId;
      applyGoogleProfileToObject(local, sync.profile);
      const committed = await commitWorkspaceToDrive(local, context.file, context.remoteWorkspace);
      applyRemoteWorkspace(committed.workspace);
      establishDriveBaseline(state, committed.file);
      clearDriveReconciliation();
      handleGoogleDriveStatus({ state: 'synced', message: hasRemote ? 'Drive replaced with this browser’s workspace' : 'Drive workspace created from this browser' });
      toast(hasRemote ? 'Drive replaced · previous Drive copy saved for recovery' : 'Drive workspace created');
    } catch (error) {
      handleGoogleDriveStatus({ state: 'error', message: error.message || 'Could not save this browser to Drive' });
      toast(error.message || 'Could not save this browser to Drive');
    } finally {
      driveSyncInFlight = false;
      updateGoogleDriveUi();
    }
  ...
  async function restoreDriveRecovery() {
    const sync = getDriveSync();
    const safety = getSyncSafety();
    if (!safety) return;
    const recovery = safety.getLatestRecovery(sync?.profile || null);
    if (!recovery?.workspace) return;
    const confirmed = window.confirm('Restore the saved recovery copy to this browser?\n\nIt will be restored locally first and will not overwrite Google Drive automatically.');
    if (!confirmed) return;
    safety.saveRecoverySnapshot(state, { reason: 'before-restore-recovery', source: 'local', profile: sync?.profile || null });
    applyRemoteWorkspace(recovery.workspace);
    markWorkspaceChanged();
    persistNow({ cloud: false });
    if (sync?.profile) safety.clearAccountRecord(sync.profile);
    driveConflictPending = Boolean(sync?.isSignedIn());
    driveReconcileContext = null;
    handleGoogleDriveStatus({ state: sync?.isSignedIn() ? 'conflict' : 'idle', message: sync?.isSignedIn() ? 'Recovery restored locally · Sync Now to review Drive' : 'Recovery restored locally' });
    updateGoogleDriveUi();
    toast('Recovery copy restored locally');
  }
